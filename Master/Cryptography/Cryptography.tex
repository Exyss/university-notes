...
\begin{document}
    ...
    \begin{frameddefn}{Symmetric-key Encryption (SKE) schemes}
        Given two functions $\mathrm{Enc} : \mathcal{K} \times \mathcal{M} \to \mathcal{C}$ and $\mathrm{Dec} : \mathcal{K} \times \mathcal{C} \to \mathcal{M}$, we say that $\Pi = (\mathrm{Enc}, \mathrm{Dec})$ is a correct Symmetric-key Encryption (SKE) scheme when:
        \[\forall m \in \mathcal{M}, \forall K \in_R \mathcal{K} \qquad \mathrm{Dec}(K, \mathrm{Enc}(K, m)) = m\]
    \end{frameddefn}
    ...
    By properties of the bit-wise XOR function, it's easy to see that OTP is correct:
    ...
    \begin{proof}
        ...
        \begin{enumerate}
            ...
            \item[3. $\implies$ 1.] Assume the third statement holds. Fix $m \in \mathcal{M}$ and $c \in \mathcal{C}$.
    ...
    We discussed how randomness can be used to generate secret keys over a probability distribution, but how can a random value be generated? As we will discuss later, randomness is a \underline{crucial} concept for secure cryptography. Clearly, there is no such concept as \textit{real randomness}: even if the whole universe may appear chaotic, everything is technically deterministic. The process of generating a random-enough value is called \textbf{randomness extraction} and it is typically achieved by measuring physical quantities (noise, air humidity, \dots) in order to produce a short unpredictable sequence of bits (e.g. 256 bits), which is expensive to generate and not necessarily uniform. This short \curlyquotes{truly random} sequence gets usually expanded to any desired length -- as long as it is polynomial with respect to the original length -- through the use of a \textbf{pseudorandom generator (PRG)}. However, this process requires some strict computational assumptions, which we'll discuss later.
    ...
    The goodness of the hash functions is measured in terms of \textbf{low collision probability}, defined as the probability that a random variable $Y$ over a set $\mathcal{Y}$ and a copy $Y'$ of $Y$ return the same result:
    \[\mathrm{Col}(Y) = \Pr[Y = Y']\]
    ...
    \begin{proof}
        ...
        \[\mathrm{SD}(Y; U) = \frac{1}{2} \sum_{y \in \mathcal{Y}} \abs{\Pr[Y = y] - \Pr[U = y]} = \frac{1}{2} \sum_{y \in \mathcal{Y}} \abs{\Pr[Y = y] - \frac{1}{\abs{\mathcal{Y}}}}\]

        For each $y \in \mathcal{Y}$, fix $q_y = \Pr[Y = y] - \frac{1}{\abs{\mathcal{Y}}}$ and $s_y = \mathrm{sign}(q_y)$. Let $\vec q = \smat{q_{y_1} & \cdots & q_{y_{\abs{\mathcal{Y}}}}}$ and  $\vec s = \smat{s_{y_1} & \cdots & s_{y_{\abs{Y}}}}$. We notice that:
        \[\mathrm{SD}(Y; U) = \frac{1}{2} \sum_{y \in \mathcal{Y}} \abs{\Pr[Y = y] - \frac{1}{\abs{\mathcal{Y}}}} = \frac{1}{2} \sum_{y \in \mathcal{Y}} q_y s_y = \frac{1}{2} \abk{\vec q, \vec s}\]

        By the \textit{Cauchy-Schwarz inequality} (which we won't prove), we get that:
        ...
        \textbf{Claim}: $\displaystyle \sum_{y \in \mathcal{Y}} q_y^2 \leq \frac{4\varepsilon^2}{\abs{\mathcal{Y}}}$

        \begin{proof}[Proof of the claim]
            Through algebraic manipulation we get that:
            \[\begin{split}
                \sum_{y \in \mathcal{Y}} q_y^2 &= \sum_{y \in \mathcal{Y}} \rbk{\Pr[Y = y] - \frac{1}{\abs{\mathcal{Y}}}}^2 \\
                ...
                &= \mathrm{Col}(Y) - \frac{1}{\abs{\mathcal{Y}}}
            \end{split}\]

            By hypothesis, we know that the collision probability of $Y$ is bounded, concluding that:
            \[\sum_{y \in \mathcal{Y}} q_y^2 = \mathrm{Col}(Y) - \frac{1}{\abs{\mathcal{Y}}} \leq \frac{1}{\abs{\mathcal{Y}}}(1+4\varepsilon^2) - \frac{1}{\abs{\mathcal{Y}}} = \frac{4\varepsilon^2}{\abs{\mathcal{Y}}}\] 
        \end{proof}

        Through the claim we directly conclude that:
        \[\mathrm{SD}(Y;U) = \frac{1}{2} \sqrt{\rbk{\sum_{y \in \mathcal{Y}} q_y^2} \abs{\mathcal{Y}}} \leq \frac{1}{2} \sqrt{\frac{4\varepsilon^2}{\abs{\mathcal{Y}}} \abs{\mathcal{Y}}} = \varepsilon\]
    \end{proof}
    ...
    \begin{proof}
        ...
        \[\begin{split}
            ...
            &\leq 2^{-d} \cdot (2^{-k} + 2^{-\ell})\\
            &= \frac{1}{2^{d+\ell}}(2^{\ell-k} + 1)
        \end{split}\]

        Finally, by hypothesis we conclude that:
    ...
    Our next objective is to overcome all these limitations (or at least reduce them). This can be achieved at the price of two very strong assumptions:
    \begin{enumerate}
    ...
    In the previous chapter we discussed how randomness is essential for good cryptography, even though true randomness cannot be achieved. To get a good enough approximation of true randomness, modern systems use \textbf{pseudo-randomness}, i.e. efficient deterministic algorithms that generate a sequence of bits that look unpredictable but aren't truly random. By definition, if enough time and resources are permitted, an adversary is clearly able to learn how a pseudorandom algorithm works through bruteforce. Hence, we'll restrict that our adversary doesn't have such time and resources, i.e. their computation must also be efficient.
    ...
    \begin{proof}
        ...
        It's easy to see that for each $i \in [0,\ell]$ the function $f_i$ is PPT-computable since $G$ is a PPT algorithm by hypothesis and $\ell = \poly(n)$. Moreover, $f_0$ is deterministic since it doesn't generate random bits. Let $H_0, \ldots, H_\ell$ be the distributions over the outputs of the functions $f_0, \ldots, f_\ell$. We observe that $H_\ell = \mathcal{U}_{n+\ell}$ since every bit is truly random in $f_\ell$. Hence, by letting $G^\ell = f_0$ we get that $H_0 = G^\ell(\mathcal{U}_n)$ and $H_\ell = \mathcal{U}_{n+\ell}$.
    ...
    To prove that the implication also holds from right to left, thus that OWF imply PRGs, we need a fundamental theoretical concept called \textbf{hard-core bits}. Given a function $f$, these bits contain information about an input $x$ that is hard to obtain from the output $f(x)$, meaning that any adversary $A$ cannot do better than guessing with 50\% chance -- up to some negligible error -- if the bit comes from $x$ or not. Hard-core bits are given by a function $h$ -- called \textit{predicate} -- that extract an hard-core bit from any input $x$.
    ...
    \begin{proof}
        ...
        \begin{proof}[Proof of the claim.]
            ...
            \begin{enumerate}
                \item $D$ challenges $A'$ to play the 1-time game (thus $D$ is the challenger and $A'$ is the adversary). $D$ will use $z$ to compute $m_b \oplus z = c$ instead of choosing a random string.
    ...
    In practical applications, many common encryption schemes (e.g. DES, 3DES, AES, \dots) are based on on \textit{pseudorandom permutations (PRP)}, i.e. PRFs that are invertible if the key is known. For now, we'll give the idea behind how PRFs can be used in theory to produce CPA-secure SKEs.
    ...
    \begin{proof}
        ...
        We now assume that the inductive hypothesis holds for $n-1$, i.e. that the GGM family $\mathcal{F'} = \{F_K' : \{0,1\}^{n-1} \to \{0,1\}^{\lambda}\}_{K \in \{0,1\}^\lambda}$ is a PRF. First, we observe that for each key $K \in \{0,1\}^\lambda$, for each $x \in \{0,1\}^n$ and for each $y \in \{0,1\}^{n-1}$ it holds that $F_K(y \concat x) = G_{x}(F_K'(y))$.
        
        We define the following three hybrid distributions:
        \begin{itemize}
            \item $\mathrm{Hyb}_0$ is the distribution over the function $F_K(y \concat x) = G_x(F_K'(y))$
            \item $\mathrm{Hyb}_1$ is the distribution over the function $H(y \concat x) = G_x(R'(y))$ where $R' \in_R \mathcal{R}(n-1 \to \lambda)$.
        ...
        \begin{proof}[Proof of Claim 1]
            ...
            \begin{enumerate}
                \item The challenger $C$ generates a key $K \in \{0,1\}^\lambda$ and sets $J = F'_K$ if $b = 0$, otherwise $J \in_R \mathcal{R}(n-1 \to \lambda)$.
                \item $A_{01}$ challenges $D_{01}$ to distinguish $\mathrm{Hyb}_0$ from $\mathrm{Hyb}_1$.
                \item For each query $y \concat x$ -- with $x \in \{0,1\}$ and $y \in \{0,1\}^n$ -- made by $D_{01}$, the adversary $A_{01}$ forwards $y$ to the challenger $C$.
        ...
        \begin{proof}[Proof of Claim 2]
            ...
            \begin{enumerate}
                ...
                \item For each query $y \concat x$ -- with $x \in \{0,1\}$ and $y \in \{0,1\}^n$ -- made by $D_{12}$, the adversary $A_{12}$ answers with $z^x_{i(y)}$, where $i(y) \in [1,t]$ is either the index that was used by $A_{12}$ if $D_{12}$ already queried $y$ or the next unused index.
    ...
    \begin{proof}
        ...
        \begin{proof}[Proof of Claim 1.]
            We reduce the distinguishability of $F_K$ from $R$ to the distinguishability of $G(\lambda, b)$ from $H(\lambda, b)$. Fix $b \in \{0,1\}$. By way of contradiction, suppose that $G(\lambda, b) \not\approx_c H(\lambda, b)$, i.e. there is an adversary $A$ that distringuished $G(\lambda, b)$ from $H(\lambda, b)$ with probability at least $n^{-c}$ for some $c > 0$.
        ...
        Now, let $H'(\lambda, b)$ be the hybrid equivalent to $H(\lambda, b)$ where applications of $R(\cdot)$ is replaced with truly random strings. It's easy to see that $H'(\lambda, 0) \equiv H'(\lambda, 1)$.

        \textbf{Claim 2}: $H(\lambda, b) \approx_c H'(\lambda,b)$ for $b \in \{0,1\}$
        ...
        The two claims conclude that $G(\lambda,0) \approx_c H(\lambda, 0) \approx_c H'(\lambda, 0) \equiv H'(\lambda, 1) \approx_c H(\lambda, 1) \approx_c G(\lambda, 1)$.
    \end{proof}

    \section{UFCMA-security and universal hash families}

    After showing that the common encryption schemes are CPA-secure, we focus on secure message authentication. Consider any tag function $\mathrm{Tag} : \mathcal{K} \times \mathcal{M} \to \mathcal{T}$. When can this function be considered secure under a computational setting? The idea is to make it hard to forge a pair $(m^*, \tau^*)$ such that $\mathrm{Tag}(K,m^*) = \tau^*$ when $K$ is unknown. This concept is formally known as \textbf{Unforgeability under Chosen-Message Attacks (UFCMA)}.
    ...
    \begin{proof}
        Let $G(\lambda) := \mathrm{Game}_{\Pi,A}^{\text{UFCMA}}(\lambda)$. Let $H(\lambda)$ be the hybrid distribution obtained by replacing $F_K(\cdot) \in \mathcal{F}$ with $R(\cdot) \in \mathcal{R}(n \to n)$ in the game $G(\lambda)$ (in other words, we're using a random function $R$ for the tag instead of $F_K$).

        Proceeding in the usual way with a simultaneous game, we can reduce the distinguishability of $F_K$ from $R$ to the distinguishability of $G(\lambda)$ from $H(\lambda)$ (omitted, similar to Claim 1 of \Cref{cpa_prf}). This concludes that $G(\lambda) \approx_c H(\lambda)$.
    ...
    \begin{proof}
        ...
        Let $H(\lambda)$ be the hybrid distribution that plays the PRF game with the function $R'(h_s(\cdot))$, where $R' \in \mathcal{R}(n \to n)$. It can be proven that $G(\lambda,0) \approx_c H(\lambda)$ in the standard way by reducing the distinguishability of $\mathrm{F}_K$ from $R'$ to the distinguishability of $G(\lambda,0)$ from $H(\lambda)$.
    ...
    \begin{frameddefn}{CBC-MAC}
        Let $\mathcal{F} = \{F_k : \{0,1\}^n \to \{0,1\}^{n}\}_{k \in \{0,1\}^\lambda}$ be a PRF family. We define the CBC-MAC family as $\mathcal{H}_{\text{CBC}} = \{h_s : \{0,1\}^{nd} \to \{0,1\}^{n}\}_{s \in \{0,1\}^\lambda}$, where:
    ...
    \begin{framedthm}{}
        ...
        \begin{itemize}
            ...
            \item $\mathcal{H}_{\text{CBC}}$ is AU for VIL
            \item $\mathcal{F}(\mathcal{H}_{\text{CBC}})$ is a PRF
            \item $\mathcal{F}(\mathcal{H}_{\text{CBC}})$ is UFCMA-secure for VIL
        \end{itemize}
    \end{framedthm}
    ...
    \begin{proof}[Proof (sketch).]
        ...
        \begin{proof}[Proof of Claim 2.]
            By way of contradiction, suppose that there is an adversary $A'$ that is able to distinguish $\mathrm{Game}_{\Pi, A}^{\text{CCA}}(\lambda, b)$ from $H(\lambda, b)$ with non-negligible probability. $A'$ returns 0 when it recognizes the CCA game, otherwise it returns 1 when it recognizes the hybrid game.
            
            We define an adversary $B'$ for the INT game:
            \begin{enumerate}
                \item $B'$ starts both games with $A'$ and $C'$
                \item $A'$ sends $q = \poly(n)$ encryption queries to $B'$. Each query $m_i$ gets forwarded to the challenger $C'$, which will answer with a chipertext $c_i$. The ciphertext is then forwarded from $B'$ to $A'$.
                \item $A'$ sends a series of decryption queries to $B'$. For each query $\widetilde c_i$, $B'$ returns $m_i$ if $(m_i, \widetilde c_i)$ is a query-answer pair of the previous encryption queries, otherwise it returns $\bot$
                \item $A'$ sends the challenge messages $m_0^*, m_1^*$ to $B'$, which then forwards $m_b^*$ to $C'$, for $b \in_R \{0,1\}$. Afterwards, $C'$ replies with $c^*$, which gets forwarded back to $A'$ by $B'$.
                \item $B'$ keeps playing the game with $A'$ using the same istructions as before (just to make $A'$ terminate). After receiving the bit $b'$ from $A'$, $B'$ samples $j \in [q]$ and returns $\widetilde c_j$ to $C'$ 
            \end{enumerate}
            
            Let $E$ be the event such that $E = 1$ if $A'$ makes a decryption query $\widetilde c$ that is both different from all the answers to the encryption queries (i.e. $\widetilde c$ is fresh) and such that $\mathrm{Dec}(K, \widetilde c) \neq \bot$. When $E = 0$ and $j$ is such that $c_j = \widetilde c$, $B'$ will always win. Therefore, $A'$ it holds that:
            \[\Pr[\mathrm{Game}_{\Pi, B'}^{\text{INT}}(\lambda) = 1] \geq \Pr[\text{$A'$ causes $E = 0$}, c_j = \widetilde c] \geq \frac{1}{n^c} \cdot \frac{1}{q} \]
            contradicting the fact that $\Pi$ satisfies integrity.
        \end{proof}
    \end{proof}
    ...
    \begin{enumerate}
        ...
        where $(K_1, K_2) \in \mathcal{K}_1 \times \mathcal{K}_2$ and $\mathrm{Dec}(K_1, c) = m \mid\mid \tau$.
    \end{enumerate}  

    In real-world implementations, Encrypt-and-MAC was used by old versions of the \textit{SSL/TLS protocol}, while new versions use Encrypt-then-MAC. Mac-then-Encrypt is used by the \textit{SSH protocol}. Out of all the three constructions, only EtM guarantees that the resulting scheme satisfies both CPA-security and integrity when $\Pi_1$ is CPA-secure and $\Pi_2$ has a particular type of UFCMA security: tags are required to be unique, i.e. for each key $K \in \mathcal{K}$ and each message $m \in \mathcal{M}$ there is an unique tag $\tau \in \mathcal{T}$ such that $\mathrm{Tag}(K, m) = \tau$.

    We observe that this property isn't as strong as it seems: every UFCMA-secure MAC scheme that we have seen until now satisfies this property (in fact, this property basically requires that tags aren't randomized).

    \begin{framedthm}{}
        Let $\Pi' = (\mathrm{Enc}', \mathrm{Dec}')$ be an EtM SKE constructed from the SKE $\Pi_1 = (\mathrm{Enc}, \mathrm{Dec})$ and the MAC $\Pi_2 = (\mathrm{Tag})$. Then, $\Pi'$ satisfies both CPA-security and integrity if $\Pi_1$ is CPA-secure and $\Pi_2$ is UFCMA-secure with unique tags.
    \end{framedthm}

    \begin{proof}
        As usual, we proceed with two reductions: one that goes from one CPA-security to the other and one that goes from integrity to UFCMA-security with unique tags.
        ...
        \begin{proof}[Proof of Claim 1.]
            ...
            \begin{enumerate}
                \item $A_1$ picks a key $K_2 \in_R \mathcal{K}_2$. This key will be used to compute tags (observe that the challenger $C_1$ also picks a key $K_1 \in_R \mathcal{K}_1$, which will be used for encryptions)
            ...
            The above reduction has no particular instruction that must be discussed: $A_1$ just acts as an intermediate between $A'$ and $C_1$, while also tagging the chipertexts sent by $C_1$. Therefore, $A_1$ is able to distinguish the CPA game for $\Pi_1$ if and only if $A'$ is able to do so for $\Pi'$, raising a contradiction on the CPA-security of $\Pi_1$.  
        \end{proof}

        \textbf{Claim 2}: $\Pi'$ satisfies integrity

        \begin{proof}[Proof of Claim 2.]
            By way of contradiction, suppose that there is an adversary $A''$ such that $\Pr[\mathrm{Game}_{\Pi', A''}^{\text{INT}}(\lambda) = 1] \geq \lambda^{-c}$ for some $c > 0$. We define a new adversary $A_2$ such that $\Pr[\mathrm{Game}_{\Pi_2, A_2}^{\text{UFCMA}}(\lambda) = 1] \geq \lambda^{-c}$:
            \begin{enumerate}
                \item $A_2$ picks a key $K_1 \in_R \mathcal{K}_1$. This key will be used to compute encryptions (observe that the challenger $C_2$ also picks a key $K_2 \in_R \mathcal{K}_2$, which will be used for taggging)
                \item $A_2$ starts the INT game with $A''$ and the UFCMA game with $C_2$
                \item For each encryption query $m_i$ from $A''$, $A_2$ computes $\mathrm{Enc}(K_1, m_i) = c_i$ and sends it to $C_2$, which replies with the tag $\tau_i$. 
                \item Upon receiving $\tau_i$, $A_2$ returns $(c_i, \tau_i)$ to $A''$
                \item After a polynomial amount of encryption queries, $A''$ sends the challenge pair $(c^*, \tau^*)$ to $A_2$, who will forward it to $C_2$.
                \item If $\mathrm{Tag}(K_2, c^*) = \tau^*$ and $c^* \neq c_i$ for all $c_i$, $A_2$ wins the UFCMA game against $C_2$, otherwise $C_2$ wins
            \end{enumerate}

            Unlike the previous claim, this reduction requires some discussion. First of all, we observe that in order to play a valid INT game the adversary $A''$ must pick a challenge pair $(c^*, \tau^*)$ such that $(c^*, \tau^*) \neq (c_i, \tau_i)$ for every encryption query $m_i$. However, this can hold true both when $c^* \neq c_i$ and when $\tau^* \neq \tau_i$ (or both). This is where UFCMA-security plays a crucial role. By way of contradiction, suppose that $c^* = c_i$ for some query. Then, by uniqueness of tags it must also hold that $\tau^* = \tau_i$, therefore $(c^*, \tau^*) = (c_i, \tau_i)$, which is absurd. Therefore, it is always guaranteed that $c^* \neq c_i$. 

            Thus, when $\mathrm{Dec}'((K_1, K_2), (c^*, \tau^*)) \neq \bot$ holds we know that both $\mathrm{Tag}(K_2, c^*) = \tau^*$ (guaranteed since the decryption works) and $c^* \neq c_i$ for all $c_i$ hold (guaranteed by the uniqueness of tags), declaring the adversary $A_2$ as winner. Otherwise, if $\mathrm{Dec}'((K_1, K_2), (c^*, \tau^*)) = \bot$ then $\mathrm{Tag}(K_2, c^*) \neq \tau^*$ must hold, declaring the challenger $C_2$ as winner.
        \end{proof}
    \end{proof}

    \begin{framedcor}{}
        Let $\Pi' = (\mathrm{Enc}', \mathrm{Dec}')$ be an EtM SKE constructed from the SKE $\Pi_1 = (\mathrm{Enc}, \mathrm{Dec})$ and the MAC $\Pi_2 = (\mathrm{Tag})$. Then, $\Pi'$ is CCA-secure if $\Pi_1$ is CPA-secure and $\Pi_2$ is UFCMA-secure with unique tags.
    \end{framedcor}
    ...
    Is a 2-layer Feistel network enough over independent PRFs to get a PRP? The answer is still no, but they almost suffice. Consider two keys $K_1, K_2 \in \{0,1\}^\lambda$. Then, for any two pairs $(x,y), (x',y') \in \{0,1\}^{2n}$ with $y = y'$ we have that:
    \[\begin{split}
        ...
        =& (x \oplus x', F_{K_2}(x \oplus F_{K_1}(y)) \oplus F_{K_2}(x' \oplus F_{K_1}(y')))
    \end{split}\]
    ...
    \begin{proof}
        ...
        \begin{proof}[Proof of Claim 3.]
            We'll use the previous lemma to prove this claim. For the following argument, it is more convinient to look at $H_2$ as given by $\psi_{R_2, R_3}(\psi_{R_1}(\cdot, \cdot))$. Consider the queries $(x_1, y_1), \ldots, (x_q, y_q)$, where $(x_i, y_i) \neq (x_j, y_j)$.Let $B$ be the event such that $B = 1$ if the outputs $\psi_{R_1}(x_i, y_i) = (x_i', y_i')$ are not \curlyquotes{$y'$-unique}, i.e. there are at least two outputs whose second halves are the same.
            ...
            \[y_i' = x_i \oplus R_1(y_i) \neq x_j \oplus R_1(y_i) = x_j \oplus R_1(y_j) = y_j'\]

            thus these queries don't affect $\Pr[B = 1]$. Now, we observe that:
            \[\begin{split}
                y_i' = y_j' &\iff x_i \oplus R_1(y_i) = x_j \oplus R_1(y_j) \\
                &\iff x_i \oplus x_i \oplus R_1(y_i) \oplus R_1(y_j) = x_i \oplus x_j \oplus R_1(y_j) \oplus R_1(y_j)\\
                &\iff R_1(y_i) \oplus R_1(y_j) = x_i \oplus x_j
            \end{split}\]
    ...
    In the theoretical context, assuming a CRH exists, the domain extension for hash families can be achieved. In particular, we study the \textbf{Merkle-Damg\aa rd transform}. The idea is to start with a CRH family and repeatedly apply it to obtain a CRH family with arbitrary domain size, preserving efficient computability.
    ...
    \begin{frameddefn}{Merkle-Damg\aa rd transform}
        ...
        \begin{itemize}
            \item The input is $x = (x_1, \ldots, x_d)$, with $x_i \in \{0,1\}^n$
    ...
    \begin{proof}
        ...
        Given the partial computations $y_1, \ldots, y_d$ and $y'_1, \ldots, y'_d$, consider the largest index $i \in [d]$ such that $(x_i, y_{i-1}) \neq (x', y'_{i-1})$  and $h_s(x_i \concat y_{i-1}) = h_s(x'_i \concat y'_{i-1})$. Notice that such index always exists since $x \neq x'$ and $H_s(x) = H_s(x')$. Then, the $i$-th application of $h_s$ gives a collision for $x_i \mid\mid y_{i-1} \neq x'\mid\mid y_{i-1}'$ since $h_s(x_i \mid\mid y_{i-1}) = h_s(x'_i \mid\mid y'_{i-1})$. Therefore, we can define a distinguisher for $\mathcal{H}$ using a distinguisher for $\mathcal{H}_{MD}$
    \end{proof}

    Observe that the above proof only works when $d > 2$ is FIL. However, it doesn't work for VIL. The flaw in the proof is given by the fact that we can't rule out that $h_s(0^{2n}) = 0^n$ may be a possibility since we don't know the structure of $h_s$. When this happens, by construction we have that:
    ...
    There are many other constructions for achieving CRH families with VIL, such as \textbf{Merkle trees}. The idea behind this construction is simple: instead of applying repeated hashes in a \curlyquotes{linear-like fashion} as done in the Merkle-Damg\aa rd transform, we apply them in a \curlyquotes{tree-like fashion}.
    ...
    In practical applications (e.g. MD5, SHA-1, SHA-2), the Merkle-Damg\aa rd transform is adapted by replacing the initial CRH  family $\mathrm{H}$ with an heuristic equivalent (similar to what is done in DES, 3-DES and AES). In the theoretical setting, instead, the initial CRH family can indeed be constructed, but it requires additional \textit{number theoretic assumptions} (e.g. the hardness of factoring, discrete log) or \textit{post-quantum assumptions}. A middle-ground solution is obtained by using AES, where $h_s(x_1, x_2) = \mathrm{AES}(x_1, x_2) \oplus x_2$. The security of this last construction can be proven only by assuming that AES is an \textit{ideal cipher}, i.e. a truly random permutation for every choice of the key.

    Hash functions are also used to build MACs. Consider the scheme $\mathrm{Tag}(K, m) = H(K \mid\mid m)$, with $H$ being hash function. Assuming that $H(\cdot) \in \mathcal{R}(\ell(n) \to n)$ and that it can be computed only by querying an oracle, the HMAC scheme is secure. If $H$ is constructed through the Merkle-Damg\aa rd transform, the scheme can be broken. The study of cryptosystems under the usage of an ideal cipher is called \textbf{Random Oracle Model (ROM)}.
    ...
    \[\Exp[I] = \sum_{i = 1}^{+\infty} i \Pr[I = i] = \sum_{i = 1}^{+\infty} i (1-\frac{1}{t})^{i-1}\frac{1}{t} = t\]

    The evaluation of $P'$ can therefore be considered efficient when $t$ is polynomial in the length $n = \log \abs{\mathcal{X}}$ of an element of $\mathcal{X}$, i.e. when:
    \[\frac{\abs{\mathcal{X}}}{\abs{\mathcal{X}'}} = t \leq \log^c \abs{\mathcal{X}} \implies \abs{X'} \geq \frac{\abs{\mathcal{X}}}{\log^c \abs{\mathcal{X}}} = \frac{2^n}{n^c}\]
    for some $c > 0$.
    ...
    \begin{framedprob}{}
        Consider the following game $\mathrm{Game}_{\Pi,A}^{\$CPA}(\lambda, b)$ for an SKE $\Pi = (\mathrm{Enc}, \mathrm{Dec})$:
        \begin{enumerate}
            \item The challenger samples $K \in_R \mathcal{K}$
            \item Upon query $m_i \in \mathcal{M}$ for $i \in [q]$ from the adversary , with $q = \poly(\lambda)$:
            \begin{itemize}
                \item If $b = 0$ the challenger answers with $c_i \in_R \mathrm{Enc}(K, m_i)$
                \item If $b = 1$ the challenger answers with $c_i \in_R \mathcal{C}$
            \end{itemize}
            \item The adversary guesses $b$ by sending $b' \in \{0,1\}$
        \end{enumerate}

        We say that an SKE is \$CPA secure when $\mathrm{Game}_{\Pi,A}^{\$CPA}(\lambda, 0) \approx_C \mathrm{Game}_{\Pi,A}^{\$CPA}(\lambda, 1)$. Prove or disprove: CPA-security is equivalent to \$CPA-security.
    \end{framedprob}

    \textit{Solution}: The two security notions are not equivalent. Consider the following SKE $\Pi = (\mathrm{Enc}, \mathrm{Dec})$ where:
    \[\mathrm{Enc}(K, m) = 0 \mid\mid \mathrm{Enc}'(K,m) \qquad \mathrm{Enc}'(K, c_0\mid\mid c_1) = \1[c_0 = 0] \cdot (\mathrm{Dec}'(K,c_1))\]

    where $\Pi' = (\mathrm{Enc}', \mathrm{Dec}')$ is CPA-secure SKE. We can easily prove that $\Pi$ is also CPA-secure through a reduction. By way of contradiction, suppose that there is an adversary $A^{\text{CPA}}_{\Pi}$ that wins the CPA game of $\Pi$ with non-distinguishable probability. We build a new adversary $A^{\text{CPA}}_{\Pi'}$ for the CPA game of $\Pi'$:
    \begin{enumerate}
        \item $A^{\text{CPA}}_{\Pi}$ makes $q = \poly(\lambda)$ encryption queries to $A^{\text{CPA}}_{\Pi'}$. For each query $m_i$, $A^{\text{CPA}}_{\Pi'}$ forwards $m_i$ to $A^{\text{CPA}}_{\Pi'}$, which returns $c_i$. $A^{\text{CPA}}_{\Pi'}$ then returns $0 \mid\mid c_i$ to $A^{\text{CPA}}_{\Pi}$
        \item The above procedure is repeated with the two challenge messages $m_0^*, m_1^*$ sent by $A^{\text{CPA}}_{\Pi}$, where $A^{\text{CPA}}_{\Pi'}$ returns the challenge ciphertext $c^*$ obtained from $C^{\text{CPA}}_{\Pi'}$
        \item After playing polynomially more encryption queries, $A^{\text{CPA}}_{\Pi}$ guesses the value of $b$ by sending $b' \in \{0,1\}$ to $A^{\text{CPA}}_{\Pi'}$, who then forwards it to $C^{\text{CPA}}_{\Pi'}$
    \end{enumerate}

    It's easy to see that this is a perfect reduction since $A^{\text{CPA}}_{\Pi'}$ acts only as an intermediate machine that adds a 0 as prefix to every ciphertext, meaning that $A^{\text{CPA}}_{\Pi'}$ forms a distinguisher for the CPA game of $\Pi'$, and thus raising a contradiction. Therefore, we conclude that $\Pi$ must be CPA-secure.

    We can also easily prove that $\Pi$ is not \$CPA secure. When $b = 1$, each of the $p = \poly(\lambda)$ ciphertexts sent by the challenger may be a string with 1 as prefix, which is never an output of $\mathrm{Enc}$. Since the probability of this happening for each ciphertext is 1/2, we can build an adversary that distinguishes the \$CPA game with non-negligible probability: if at least one of the ciphertexts has 1 as prefix, return $b' = 1$ as final answer, otherwise return $b' = 0$.

    \begin{framedprob}{}
        Let $\Pi = (\text{Enc, Dec})$ be a SKE scheme over key space $\mathcal{K}$, message space $\mathcal{M}$, and ciphertext space $\mathcal{C}$. Formally define a variant of semantic security (alternative name for one-time security) for $\Pi$, where the messages $m_0, m_1 \in \mathcal{M}$ are chosen at random by the challenger. Then:
    
        \begin{enumerate}
            \item Suppose that $\mathcal{M} = \{0,1\}^\ell$. Assuming that $\Pi$ is semantically secure for random messages, show how to construct a SKE scheme $\Pi^* = (\text{Enc}^*, \text{Dec}^*)$ over key space $\mathcal{K}$ and message space $\mathcal{M}$ that is semantically secure in the ordinary sense.
            
            \item Show that there exists a SKE scheme $\overline{\Pi}$ that is semantically secure for random messages, but is not semantically secure in the ordinary sense.
        \end{enumerate}
    \end{framedprob}

    \textit{Solution}:

    Let $\mathrm{Game}_{\Pi,A}^{\text{SSRM}}(\lambda, b)$ be the game identical to the one-time game, except that $m_0^*, m_1^*$ are sampled uniformly-at-random by the adversary. We say that $\Pi$ is semantically secure for random messages (SSRM) when $\mathrm{Game}_{\Pi,A}^{\text{SSRM}}(\lambda, 0) \approx_c \mathrm{Game}_{\Pi,A}^{\text{SSRM}}(\lambda, 1)$.

    \begin{enumerate}
        \item Suppose that $\Pi$ is semantically secure for random messages. We construct a new SKE $\Pi^*$ as follows:
        \[\mathrm{Enc}^*(K, m) = (\mathrm{Enc}(K, r), m \oplus r) \qquad \mathrm{Dec}^*(K,(c_0, c_1)) = c_1 \oplus \mathrm{Dec}(K,c_0)\]
        where $r \in_R \mathcal{U}_n$.

        By way of contradiction, suppose that there is an adversary $A_{\Pi^*}$ that breaks one-time security for $\Pi^*$. We construct a new adversary $A_{\Pi}$ for the SSRM game of $\Pi^*$ as follows:
        \begin{enumerate}
            \item $A_{\Pi^*}$ sends the challenge messages $m_0^*, m_1^*$ to $A_{\Pi}$
            \item $A_{\Pi}$ samples $r_0, r_1 \in_R \mathcal{U}_n$ and sends them to the challenger $C_{\Pi}$, who then replies with $c^*$
            \item $A_{\Pi}$ samples $\beta \in_R \mathcal{U}_1$ and sends $(c^*, m_\beta^* \oplus r_\beta)$ to $A_{\Pi^*}$, who then replies with the bit $b'$. $A_{\Pi}$ forwards $b'$ to $C_{\Pi}$
        \end{enumerate}

        When $\beta = b$ holds, the reduction is perfect since $(c^*, m_\beta^* \oplus r_\beta) = (\mathrm{Enc}(K, r_b), m_b^* \oplus r_b)$, allowing $A_{\Pi}$ to distinguish $b$ with probability at least $n^{-c}$, for some $c > 0$. Therefore, we have that:
        \[\Pr[A_{\Pi} \text{ wins}] \geq \Pr[A_{Pi^*} \text{ wins} \mid \beta = b] \Pr[\beta = b] \geq \frac{1}{2n^c}\]
        contradicting the fact that $\Pi$ is SSRM secure.

        \item Let $\overline{\Pi} = (\overline{\mathrm{Enc}}, \overline{\mathrm{Dec}})$ be the SKE defined as follows:
        \[\overline{\mathrm{Enc}}(K,m) = \soe{ll}{
            K & \text{if } m = 0^n \\
            \mathrm{Enc}'(K,m) & \text{otherwise}
        } \qquad \overline{\mathrm{Dec}}(K,c) = \soe{ll}{
            0^n & \text{if } c = K \\
            \mathrm{Dec}'(K,c) & \text{otherwise}
        } \]

        where $\Pi' = (\mathrm{Enc}', \mathrm{Dec}')$ is any other SKE. We can immediately see that $\overline{\Pi}$ is not one-time secure since after querying $m_0^*, m_1^* \in \mathcal{M}$ such that $m_0^* = 0^n$ we get full access to the key when the challenger chooses the first message.

        Conider now the SSRM game of $\overline{\Pi}$. Let $G(\lambda, b) = \mathrm{Game}_{\Pi,A}^{\text{SSRM}}(\lambda, b)$. When $m_0^*, m_1^* \neq 0^n$, we have that $G(\lambda, 0) \equiv G(\lambda, 1)$ since the two messages are randomly picked. Therefore, we have that:
        \[\mathrm{SD}(G(\lambda, 0); G(\lambda, 1)) \leq \Pr[m_0^* = 0^n \lor m_1^* = 0^n] \leq \Pr[m_0^* = 0^n] + \Pr[m_1^* = 0^n] = \frac{2}{2^n}\]
        and thus that $G(\lambda, 0) \approx_c G(\lambda, 1)$.
    \end{enumerate}
    ...
    \begin{itemize}
        \item $(\G, +)$ is a group if $+$ satisfies associativity, the existence of a neutral element and the existence of an inverse element 
        \item $(\G, +, \cdot)$ is a ring if $(\G, +)$ is a group and $\cdot$ satisfies associativity, the existence of a neutral element and distributivity
        \item $(\G, +, \cdot)$ is a field if $(\G, +)$ is a group and $\cdot$ satisfies associativity, the existence of a neutral element, distributivity and the existence of an inverse element (except for the neutral element of $+$).
    \end{itemize}
    ...
    We observe that this algorithm can also be extended to express the GCD as a \textit{Bézout identity}, meaning that $\gcd(a,b) = ak+bh$ for some $k,h \in \Z$. This can be achieved by simply inverting each iteration made by the algorithm. For instance, in our previous example we have that:
    \begin{enumerate}
    ...
    \begin{proof}
        Consider the group $\mathbb{QR}_p$ of quadratic residues over $p$, defined as $\mathbb{QR}_p = \{y \in \Z_p^* \mid \exists x \in \Z_p^* \; y \equiv x^2 \pmod p\}$. Equivalently, we have that $\mathbb{QR}_p = \{y \in \Z_p^* \mid \exists k \in \Z \; y \equiv g^{2k} \pmod p\}$, i.e. that $\mathbb{QR}_p$ is the set of invertible numbers generated by an even exponent.

        \textbf{Claim}: $y \in \mathbb{QR}_p$ if and only if $y^{\frac{p-1}{2}} \equiv 1 \pmod p$

        \begin{proof}
            Suppose that $y \in \mathbb{QR}_p$, meaning that $y \equiv g^{2k} \pmod p$ for some $k \in \Z$. Then, we have that:
            \[y^{\frac{p-1}{2}} \equiv (g^{2k})^{\frac{p-1}{2}} \equiv (g^{p-1})^k \equiv 1 \pmod p\]

            Vice versa, suppose that $y \notin \mathbb{QR}_p$, meaning that $y \equiv g^{2k'+1} \pmod p$ for some $k' \in \Z$. Then, we have that:
    ...
    The above implies that we require a new type of cyclic group for which the DDH assumption is believed to hold. Luckily, there are many groups for which this is true. The easiest fix is to set $\G = \mathbb{QR}_{p}$ where $p = 2q+1$ and $p,q \in \Primes$ to break the previous proposition. Another very popular choice is \textit{elliptic curve groups}. An elliptic curve is composed of solutions to an equation $y = x^3 + ax^2+bx+c$, for some coefficients $a,b,c \in \R$. The elliptic curve group modulo $p \in \Primes$ is a group whose elements are the points of an elliptic curve with coordinates modulo $p$.
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[->,>=stealth,shorten >=1pt,auto,node distance=1cm,thick,main node/.style={scale=0.8,circle,draw,font=\sffamily\normalsize}]
            ...
            \node[] (6) [below of = 1, yshift = -10]{};
            \node[] (7) [below of = 4, yshift = -10]{};
    ...
    \begin{framedalgo}{ElGamal PKE scheme}
        ...
        \begin{itemize}
            ...
            \item $\mathrm{Dec}(sk, (c_1, c_2)) = c_2 (c_1)^{-sk}$ 
        \end{itemize}
    \end{framedalgo}
    ...
    \begin{proof}
        Let $G(\lambda,b) = \mathrm{Game}_{\Pi,A}^{\text{CPA}}(\lambda, b)$. We define an hybrid $H(\lambda, b)$ where $\mathrm{Enc}_H(pk, m) = (g^r, m (g^z))$, for some $z \in_R \Z_q$, instead of $\mathrm{Enc}_G(pk, m) = (g^r, m(pk)^r)$, for some $r \in_R \Z_q$. Assume that the DDH assumption holds for $\G$ and fix $b \in \{0,1\}$. By way of contradiction, suppose that there is an adversary $A$ that distinguishes $G(\lambda,b)$ from $H(\lambda,b)$ with non-negligible probability. We define a new adversary $A'$ for the DDH assumption:
        \begin{enumerate}
            \item The challenger $C$ sends $(\G,g,q, X, Y, Z)$ to $A'$, with $Z$ being either $g^{xy}$ or $g^z$ for some $z \in_R \Z_q$ (recall that $A$ doesn't know that $X = g^x$ and $Y = g^y$). $A'$ forwards $pk = X$ to $A$.
            \item $A$ sends the challenge $m_0^*, m_1^*$ to $A'$, who replies with $(Y, Z \cdot m_b)$
            \item $A$ sends a guessing bit $b'$, which gets forwarded to $C$
        \end{enumerate}
    ...
    \begin{proof}
        Given as exercise (\Cref{cpa_tdp_ex}).
    \end{proof}
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[->,>=stealth,shorten >=1pt,auto,node distance=1cm,thick,main node/.style={scale=0.8,circle,draw,font=\sffamily\normalsize}]
            ...
            \node[] (x) [right of = 2, xshift = 50]{};
            \node[draw, rectangle, minimum width=40] (3) [right of = x, xshift = 50]{$\mathrm{Vrfy}(pk,m,\sigma)$};
            \node[] (4) [right of = 3, xshift = 75]{Bob};
            \node[] (5) [below of = x, yshift = -10]{$(sk,pk)$};
            \node[] (6) [below of = 1, yshift = -10]{};
            \node[] (7) [below of = 4, yshift = -10]{};
    ...
    \begin{framedalgo}{FDH DS scheme}
        ...
        \begin{itemize}
            \item $(pk, sk) \in_R \mathrm{KGen}(1^{\lambda})$
            \item $\mathrm{Sign}(sk, m) = f^{-1}(sk, H(m))$
    ...
    \begin{framedprob}[label={cpa_tdp_ex}]{}
        Prove \Cref{cpa_tdp}
    \end{framedprob}

    \begin{proof}
        By way of contradiction, suppose that there is a distinguisher $A^{\text{CPA}}_{\Pi}$ for the CPA game of $\Pi$ with probability at least $n^{c}$, for some $c > 0$. We build a new adversary $A^{\text{HPC}}_{h_{pk}}$ for the hard-core predicate game of $h_{pk}$:
        \begin{enumerate}
            \item The challenger $C^{\text{HPC}}_h$ generates $(pk, sk)$ and sends the pair $(pk, y)$ to $A^{\text{HPC}}_{h_{pk}}$, where $y$ is an output of $f_{pk}$
            \item $A^{\text{HPC}}_{h_{pk}}$ forwards $pk$ to $A^{\text{CPA}}_{\Pi}$
            \item $A^{\text{CPA}}_{\Pi}$ sends the challenge bits $m_0^*, m_1^*$. $A^{\text{HPC}}_{h_{pk}}$ generates a random bit $b^{\#} \in_R \mathcal{U}_1$ and returns the pair $(y, b^{\#})$
            \item $A^{\text{CPA}}_{\Pi}$ guesses $b' \in \{0,1\}$. $A^{\text{HPC}}_{h_{pk}}$ returns $b^{\#} \oplus m_{b'}$ to $C^{\text{HPC}}_h$
        \end{enumerate}

        Given $y = f_{pk}(x)$, we observe that $b^{\#} = h_{pk}(x) \oplus m_b$ happens with probability 1/2 since $b^{\#} \oplus m_b$ has the same distribution as $\mathcal{U}_1$ and $h_{pk}(x)$ is indistinguishable from a random bit by definition of hard-core predicate. Now, observe that:
        \[\begin{split}
            \Pr[\text{$A^{\text{HPC}}_{h_{pk}}$ wins} \mid b^{\#} \oplus m_b = h_{pk}(x)] &= \Pr[b^{\#} \oplus m_{b'} = h_{pk}(x) \mid b^{\#} \oplus m_b = h_{pk}(x)] \\
            &= \Pr[b' = b \mid b^{\#} = h_{pk}(x) \oplus m_b] \\
            &\geq \frac{1}{2}+\frac{1}{n^c} \\
        \end{split}\] 

        When conditioning on the opposite event, instead, the distinguisher $A^{\text{CPA}}_{\Pi}$ gets no additional information, meaning that it can guess $b \neq b'$ with probability 1/2. Therefore, we have that:
        \[\begin{split}
            \Pr[\text{$A^{\text{HPC}}_{h_{pk}}$ wins} \mid b^{\#} \oplus m_b \neq h_{pk}(x)] &= \Pr[b^{\#} \oplus m_{b'} = h_{pk}(x) \mid b^{\#} \oplus m_b \neq h_{pk}(x)] \\
            &= \Pr[b' \neq b \mid b^{\#} \neq h_{pk}(x) \oplus m_b] \\
            &= \frac{1}{2} \\
        \end{split}\] 
        Using total probability, we conclude that:
        \[\Pr[\text{$A^{\text{HPC}}_{h_{pk}}$ wins}] \geq \frac{1}{2} \rbk{\frac{1}{2}+\frac{1}{n^c}} + \frac{1}{2} \cdot \frac{1}{2} = \frac{1}{2}+\frac{1}{n^c}\]

        contradicting the fact that $h$ is an hard-core predicate for $f_{pk}$.
    \end{proof}

    \begin{framedprob}{}
        Let $\Pi = (\text{KGen, Sign, Vrfy})$ be a signature scheme over message space $\mathcal{M} = \{0, 1\}^n$. Consider the following derived signature scheme $\Pi' = (\text{KGen, Sign', Vrfy'})$ over $\mathcal{M}$:
        \[ \text{Sign}'(sk, m) = (r, \text{Sign}(sk, m \oplus r), \text{Sign}(sk, r)) \text{ for } r \leftarrow_s \{0, 1\}^n \]
        \[ \text{Vrfy}'(pk, m, (r, \sigma_0, \sigma_1)) = (\text{Vrfy}(pk, m \oplus r, \sigma_0)) \land (\text{Vrfy}(pk, r, \sigma_1)). \]

        Prove or disprove: The above signature scheme $\Pi'$ is UFCMA-secure assuming $\Pi$ is UFCMA-secure.
    \end{framedprob}

    \textit{Solution}: No, the scheme $\Pi$ is not UFCMA-secure. To prove this, consider the following adversary $A$:
    \begin{enumerate}
        \item The challenger $C$ sends the public key $pk$ to $A$
        \item $A$ sends a message $m_1$ to $C$, which replies with the sign $\sigma_1' = (r_1, \sigma^{(1)}_0, \sigma_1^{(2)})$
        \item $A$ sends the message $m_2 = m_1 \oplus r$ to $C$, which replies with the sign $\sigma_2' = (r_2, \sigma^{(1)}_0, \sigma_1^{(2)})$
        \item $A$ challenges $C$ with the pair $(m_1 \oplus r_2, (r_1, \sigma^{(2)}_0, \sigma^{(1)}_1))$
    \end{enumerate}

    Since the challenger replies the sign queries correctly, we know that:
    \begin{itemize}
        \item $\sigma_1' = (r_1, \sigma^{(1)}_0, \sigma_1^{(2)}) = (r_1, \mathrm{Sign}(sk, m_1 \oplus r_1), \mathrm{Sign}(sk, r_1))$
        \item $\sigma_2' = (r_2, \sigma^{(1)}_0, \sigma_1^{(2)}) = (r_2, \mathrm{Sign}(sk, m_1 \oplus r_1 \oplus r_2), \mathrm{Sign}(sk, r_2))$
    \end{itemize}

    Therefore, we have that:
    \[\begin{split}
        &\text{Vrfy}'(pk, m_1 \oplus r_2, (r_1, \sigma^{(2)}_0, \sigma^{(1)}_1)) \\
        =& (\text{Vrfy}(pk, m_1 \oplus r_2 \oplus r_1, \sigma_0^{(2)})) \land (\text{Vrfy}(pk, r_1, \sigma_1^{(1)})) \\
        =& (\text{Vrfy}(pk, m_1 \oplus r_2 \oplus r_1, \mathrm{Sign}(sk, m_1 \oplus r_1 \oplus r_2))) \land (\text{Vrfy}(pk, r_1, \mathrm{Sign}(sk, r_1))) \\
        =& 1
    \end{split}\]

    \begin{framedprob}{}
        Let $\Pi = (\mathrm{KGen}, \mathrm{Sign}, \mathrm{Vrfy})$ be a digital signature scheme over message space $\mathcal{M}$. We say that $\Pi$ satisfies strong binding if no efficient adversary can win the following game with more than a negligible probability: upon an empty input, the adversary outputs $(pk, m, pk', m', \sigma)$ and wins if and only if $(pk, m) \neq (pk', m')$ and $\mathrm{Vrfy}(pk, m, \sigma) = \mathrm{Vrfy}(pk', m', \sigma) = 1$. Show how to combine any collision-resistant family of hash functions $\mathcal{H}$ with an UFCMA digital signature scheme $\Pi$ into a digital signature scheme $\Pi^*$ that satisfies both UFCMA and strong binding. (The message space of $\Pi^*$ should be $\mathcal{M}$, but signatures in $\Pi^*$ may be longer than signatures in $\Pi$.)
    \end{framedprob}

    \textit{Solution}: Let $\Pi' = (\mathrm{KGen}', \mathrm{Sign}', \mathrm{Vrfy}')$ be a digital signature scheme and let $\mathcal{H} = \{H_s : \{0,1\}^{\lambda+n} \to \{0,1\}^n \}_{s \in \{0,1\}^{\lambda}}$ be an hash family. We define the digital signature scheme $\Pi_{\mathcal{H}} = (\mathrm{KGen}, \mathrm{Sign}, \mathrm{Vrfy})$ as follows:
    \begin{itemize}
        \item $s \in_R \{0,1\}^{\lambda}$ is a fixed seed
        \item $\mathrm{KGen}(1^\lambda) = (pk, (pk, sk))$ where $(pk,sk) \in_R \mathrm{KGen}'(1^{\lambda})$
        \item $\mathrm{Sign}((pk,sk), m) = (h, \mathrm{Sign}'(sk, h))$, where $h = H_s(pk \mid\mid m)$
        \item $\mathrm{Vrfy}(pk, m, (h, \sigma)) = \1[h = H_s(pk \mid\mid m)] \cdot \mathrm{Vrfy}'(pk, h, \sigma)$
    \end{itemize}

    We prove with two claims that $\Pi_{\mathcal{H}}$ is UFCMA-secure with strong binding when $\Pi'$ is UFCMA-secure and $\mathcal{H}$ is collision-resistant.

    \textbf{Claim 1}: $\Pi_{\mathcal{H}}$ is UFCMA-secure when $\Pi$ is UFCMA-secure

    \begin{proof}[Proof of Claim 1.]
        By way of contradiction, suppose that there is an adversary $A^{\text{UFCMA}}_{\Pi_{\mathcal{H}}}$ that breaks the UFCMA-security of $\Pi_{\mathcal{H}}$. We define a new adversary $A^{\text{UFCMA}}_{\Pi'}$ for the UFCMA game of $\Pi'$ as follows:
        \begin{enumerate}
            \item $A^{\text{UFCMA}}_{\Pi'}$ samples a seed $s \in_R \{0,1\}^{\lambda}$
            \item The challenger $C^{\text{UFCMA}}_{\Pi'}$ sends the public key $pk$ to $A^{\text{UFCMA}}_{\Pi'}$, who then forwards it to $A^{\text{UFCMA}}_{\Pi_{\mathcal{H}}}$
            \item $A^{\text{UFCMA}}_{\Pi_{\mathcal{H}}}$ makes $q = \poly(\lambda)$ signature queries to $A^{\text{UFCMA}}_{\Pi'}$. For each query $m_i$, $A^{\text{UFCMA}}_{\Pi'}$ forwards $H_s(pk \mid\mid m_i)$ to $C^{\text{UFCMA}}_{\Pi'}$. $C^{\text{UFCMA}}_{\Pi'}$ replies with $\sigma_i$, which gets forwarded back to $A^{\text{UFCMA}}_{\Pi_{\mathcal{H}}}$
            \item $A^{\text{UFCMA}}_{\Pi_{\mathcal{H}}}$ sends a pair $(m^*, (h^*,  \sigma^*))$ to $A^{\text{UFCMA}}_{\Pi'}$, who then forwards $(h^*, \sigma^*)$ to $C^{\text{UFCMA}}_{\Pi'}$,
        \end{enumerate}

        When $A^{\text{UFCMA}}_{\Pi_{\mathcal{H}}}$ correctly guesses $(m^*, (h^*,\sigma^*))$ for $\Pi_{\mathcal{H}}$, meaning that $\mathrm{Vrfy}(pk, m^*, (h^*, \sigma^*)) = 1$, by definition we know that $h^* = H_s(pk \mid\mid m^*)$ and $\mathrm{Vrfy}'(pk, h^*, \sigma^*) = 1$ must hold. Therefore, the pair $(h^*, \sigma^*)$ will also be a correct guess for $\Pi'$. Since this happens with non-negligible probability, $A^{\text{UFCMA}}_{\Pi'}$ breaks the UFCMA-security of $\Pi'$, raising a contradiction.
    \end{proof}

    \textbf{Claim 2}: $\Pi_{\mathcal{H}}$ has strong binding when $\mathcal{H}$ is a CRH family

    \begin{proof}[Proof of Claim 2.]
        By way of contradiction, suppose that there is an adversary $B^{\text{SB}}_{\Pi_{\mathcal{H}}}$ that breaks the strong binding of $\Pi_{\mathcal{H}}$. We define a new adversary $B^{\text{CRH}}_{\mathcal{H}}$ for the CRH game of $\mathcal{H}$ as follows:
        \begin{enumerate}
            \item The challenger $C^{\text{CRH}}_{\mathcal{H}}$ sends a seed $s \in_R \{0,1\}^\lambda$ to $B^{\text{CRH}}_{\mathcal{H}}$, who then forwards it to $B^{\text{SB}}_{\Pi_\mathcal{H}}$
            \item $B^{\text{CRH}}_{\mathcal{H}}$ sends an empty message to $B^{\text{SB}}_{\Pi_\mathcal{H}}$, who replies with $(pk, m, pk', m', (h, \sigma))$.
            \item $B^{\text{CRH}}_{\mathcal{H}}$ sends $(pk \mid\mid m, pk' \mid\mid m')$ to $C^{\text{CRH}}_{\mathcal{H}}$
        \end{enumerate}

        When $B^{\text{SB}}_{\Pi_{\mathcal{H}}}$ correctly guesses $(pk, m, pk', m', (h, \sigma))$, by definition we have that:
        \begin{itemize}
            \item $\mathrm{Vrfy}(pk, m, (h, \sigma)) = 1$ implies that $h = H_s(pk \mid\mid m)$ and $\mathrm{Vrfy}'(pk, h, \sigma) = 1$
            \item $\mathrm{Vrfy}(pk', m', (h, \sigma)) = 1$ implies that $h = H_s(pk' \mid\mid m')$ and $\mathrm{Vrfy}'(pk', h, \sigma) = 1$
        \end{itemize}
        Since $(pk, m) \neq (pk', m')$ and $H_s(pk \mid\mid m) = H_s(pk' \mid\mid m')$, the two strings $pk \mid\mid m \neq pk' \mid\mid m'$ form a collision on $H_s$. Therefore, $B^{\text{CRH}}_{\Pi}$ find a collision on $H_s$ with non-negligible probability, contradicting the collision-resistance of $\mathcal{H}$
    \end{proof}

    \begin{framedprob}{}
        Suppose that $t$ receivers publish their public keys $pk_1, \ldots, pk_t$. Alice wants to send an encrypteed message to one of the receivers in such a way that no adversary can tell which of the $t$ receivers is the intended recipient of the message.
        \begin{enumerate}
            \item Define a security model that captures this requirement for the base case $t = 2$. A PKE scheme that has this property is said to be an anonymous PKE scheme. Explain why considering $t = 2$ is sufficient.
            \item Show that the ElGamal PKE scheme is anonymous under the DDH assumption
            \item Show that the RSA PKE scheme is not anonymous (even under the RSA assumption)
        \end{enumerate}
    \end{framedprob}
    ...
    \begin{enumerate}
        \item Let $\mathrm{Game}_{\Pi, A}^{\mathrm{ANON}}(\lambda, t, j)$ be the game defined as follows:
        \begin{enumerate}
            \item The challenger $C$ sends $t$ different public keys $pk_0, \ldots, pk_{t-1}$
            \item The adversary sends a message $m^*$ and the challenger replies with a ciphertext $c^*$ obtained by encrypting $m^*$ with $pk_j$
            \item The adversary guesses $j$ by sending $j'$. The adversary wins if and only if $j = j'$.
        \end{enumerate}
        where $0 \leq j \leq t-1$ and $t \geq 2$. It's easy to see that the $t$-player game can be simulated by playing multiple instances of the 2-player game (using the pairs $(pk_0,pk_1), (pk_1, pk_2), \ldots, (pk_{t-1}, pk_t))$. Therefore, we say that $\Pi$ is anonymous when $\mathrm{Game}_{\Pi, A}^{\mathrm{ANON}}(\lambda, 2, 0) \approx_c \mathrm{Game}_{\Pi, A}^{\mathrm{ANON}}(\lambda, 2, 1)$. 

        \item Let $\Pi$ be the ElGamal PKE. By way of contradiction suppose that there is an adversary $A^{\text{ANON}}_{\Pi}$ that breaks anonymity for $\Pi$. We define an adversary $A^{\text{DDH}}$ for the DDH game as follows:
        \begin{enumerate}
            \item The challenger $C^{\text{DDH}}$ sends the tuple $(\G, g, q, X, Y, Z)$ to $A^{\text{DDH}}$, who then forwards $(X,Y)$ to $A^{\text{ANON}}_{\Pi}$
            \item $A^{\text{ANON}}_{\Pi}$ sends $m^*$. $A^{\text{DDH}}$ samples $W \in_R \{X,Y\}$ and sends $(W, m^* Z)$ to $A^{\text{ANON}}_{\Pi}$
            \item $A^{\text{ANON}}_{\Pi}$ sends $b'$ to $A^{\text{DDH}}$, who then forwards it to $C^{\text{DDH}}$
        \end{enumerate}

        Assume that $X = g^x$ and $Y = g^y$ for $x,y \in_R \Z_q$. Observe that when $Z = g^{xy}$, then $(W, m^* Z) = (g^x, m^* Y^x)$ when $W = X$, while $(W, m^* Z) = (g^y, m^* X^y)$ when $W = X$. Therefore, we have that:
        \[\begin{split}
            \Pr[b' = b \mid Z = g^{xy}] &= \frac{1}{2} \Pr[b' = b \mid Z = g^{xy}, W = g^x] + \frac{1}{2} \Pr[b' = b \mid Z = g^{xy}, W = g^y] \\
            &\geq \frac{1}{2} \frac{1}{n^c} + \frac{1}{2} \frac{1}{n^c}
        \end{split}\]

        When $Z = g^z$ with $z \in_R \Z_q$, the adversary $A^{\text{ANON}}_{\Pi}$ gets no additional information since the ciphertext is actually invalid. Therefore, we have that $\Pr[b' = b \mid Z = g^{z}] = \frac{1}{2}$, concluding that:
        \[\abs{\Pr[b' = b \mid Z = g^{xy}] - \Pr[b' = b \mid Z = g^z]} \geq \frac{1}{2} + \frac{1}{n^c}\]
        breaking the DDH assumption on $(\G, g, q)$.

        \item Consider the following adversary $A$ for the ANON game of RSA, defined as follows:
        \begin{enumerate}
            \item The challenger sends $pk_0 =(n_0, e_0)$ and $pk_1 = (n_1, e_1)$
            \item The adversary sends a message $m^*$ and gets $c^*$ as answer
            \item The adversary returns $b' = \1[c^* \geq n_0]$
        \end{enumerate}
        
        W.l.o.g. assume $n_0 < n_1$. When $b = 0$ the condition $c^* \geq n_0$ is always false, therefore $\Pr[A \text{ returns } 0 \mid b = 0] = \Pr[c^* < n_0 \mid b= 0 ] =  1$. When $b = 1$, instead, we have that
        $$\Pr[A \text{ returns } 1 \mid b = 1] = \Pr[c^* \geq n_0 \mid b= 1 ] =  1 - \frac{n_1 - n_0}{n_1} = \frac{n_0}{n_1} \geq  \frac{1}{4}$$

        where the latter inequality comes from the fact that $2^{2\lambda-2} \leq n_0, n_1 \leq 2^{2\lambda}$ since they are the product of two pairs of primes each having approximately $\lambda$ bits. Therefore, we conclude that:
        $$\Pr[A \text{ wins }] = \frac{1}{2} \Pr[A \text{ returns } 0 \mid b = 0] + \frac{1}{2} \Pr[A \text{ returns } 1 \mid b = 1]  \geq \frac{1}{2} \cdot 1 + \frac{1}{2} \cdot \frac{1}{4}= \frac{5}{8}$$

        proving that RSA is not anonymous
    \end{enumerate}

    \begin{framedprob}{}
        Let $\mathrm{Game}_{\Pi, A}^{\text{CCA-1}}(\lambda, b)$ be the game identical to the CCA game for PKE schemes but where decryption queries are allowed only \textit{before} receiving the challenge ciphertext. Prove or disprove the following:
        \begin{enumerate}
            \item If a scheme is CPA-secure then it is also CCA-1 secure
            \item If a scheme is CCA-1-secure then it is also CPA secure
            \item If a scheme is CCA-1-secure then it is also CCA secure
        \end{enumerate}
    \end{framedprob}

    \textit{Solution}:
    \begin{enumerate}
        \item The claim is false. Let $\Pi = (\mathrm{KGen}, \mathrm{Enc}, \mathrm{Dec})$ be any CPA-secure PKE. We define a new PKE $\Pi' = (\mathrm{KGen}', \mathrm{Enc}', \mathrm{Dec}')$ as follows:
        \begin{itemize}
            \item $\mathrm{KGen}' = \mathrm{KGen}$
            \item $\mathrm{Enc}' = \mathrm{Enc}$
            \item $\mathrm{Dec}'(sk, c) = sk$ when $c = 0^n$ and $\mathrm{Dec}'(sk, c) = \mathrm{Dec}(sk, c)$ otherwise
        \end{itemize}

        The CPA-security of $\Pi'$ follows immediately from the CPA-security of $\Pi$ since they share the same encryption function and key generator. However, we can define an adversary $A$ for the CCA-1 game of $\Pi'$ as follows:
        \begin{enumerate}
            \item The challenger $C$ sends the public key $pk$
            \item $A$ sends the decryption query $c' = 0^n$ and gets $m'$ as answer
            \item $A$ sends two challenge messages $m_0^* \neq m_1^*$ and gets the ciphertext $c^*$ as answer
            \item $A$ sends $b' = \1[\mathrm{Dec}(m', m_1^*) = c^*]$ as final guess
        \end{enumerate} 

        By definition of $\mathrm{Dec}'$, we have that $m' = sk$, therefore the adversary will win with probability close to 1. This concludes that $\Pi'$ is not CCA-1-secure, diproving the claim.

        \item The claim is true. This immediately follows from the definition of the two security notions since CCA-1-security contains CPA-security. This can be formally proven through a simple reduction between the two games that ignores the decryption queries.
        
        \item This claim is definitely false but I can't find a counterexample. 

    \end{enumerate}
    
    \chapter{Identification schemes}

    \section{$\Sigma$-protocols and zero-knowledge}
    ...
    \begin{enumerate}
        ...
        \item $A$ plays the $A_1(\tau_1, \ldots, \tau_q) = s$, where $s$ is a \curlyquotes{state}
    ...
    \begin{framedalgo}{Schnorr's $\Sigma$-protocol}
        The Schnorr $\Sigma$-protocol $\Sigma_{\text{Schorr}} = (\mathrm{KGen}, P, V)$ is defined as follows:
        \begin{itemize}
            ...
            \begin{enumerate}
                ...
                \item $P_2(pk,sk,\alpha, \beta) = \gamma$ such that $\gamma = \beta x + a$
    ...
    \[g^\gamma = g^{\beta x + a} = \alpha (pk)^\beta\]
    ...
    To show the passive security of this protocol, we have to introduce two stronger properties that when summed up give passive security as a natural byproduct: \textbf{Honest-verifier Zero-knowledge (HVZK)} and \textbf{Special Soundness (SS)}. The first property tells us that real transcripts computed by the protocol are computationally indistinguishable from \textit{simulated transcripts} produced by a simulation algorithm. In other words, the transcripts don't give any information to the verifier.
    ...
    \begin{framedthm}{HVZK$+$SS implies passive security}
        Let $\Sigma = (\mathrm{KGen}, P, V)$ be a $\Sigma$-protocol. Then, $\Sigma$ has passive security if it has HVZK and special soundness.
    \end{framedthm}

    \begin{proof}
        Let $G_0(\lambda) = \mathrm{Game}^{\text{ID}}_{\Sigma, A}$ and let $G_1(\lambda)$ be game played as $G_0(\lambda)$ but where transcript queries answers are taken from $\mathrm{Sim}(pk)$ instead of $P(pk,sk) \rightleftarrows V(pk)$.

        Let $q = \poly(\lambda)$ be the number of transcript queries made in the two games. We define $q+1$ hybrids $H_0, \ldots, H_q$ such that each $H_i$ answers  the first $i$ queries using $\mathrm{Sim}(pk)$ instead of $P(pk,sk) \rightleftarrows V(pk)$. By definition, we have that $G_0(\lambda) = H_0(\lambda)$ and $G_1(\lambda) = H_q(\lambda)$.

        \textbf{Claim 1}: $H_i \approx_c H_{i+1}$ for all $i \in [q-1]$

        \begin{proof}{Proof of Claim 1.}
            We prove the claim through a reduction from the HVZK property. By way of contradiction, suppose that there is an adversary $A$ that distinguishes $H_i(\lambda)$ from $H_{i+1}(\lambda)$ with non-negligible probability.. We build a new adversary $A'$ defined as follows:
            \begin{enumerate}
                \item The challenger $C$ sends $(pk, sk, \tau)$ to $A'$, who then forwards $pk$ to $A$
                \item $A$ makes $q$ transcript queries to $A'$. For each $j$-th query, $A'$ answers with $\tau_j$ such that:
                \[\tau_j \in_R \soe{ll}{
                    \mathrm{Sim}(pk) & \text{ if } j \leq i \\
                    \{\tau\} & \text{ if } j = i+1 \\
                    P(pk,sk) \rightleftarrows V(pk) & \text{ if } j > i+1 \\
                }\]
                \item $A$ guesses the value of $b$ by sending $b' \in \{0,1\}$ to $A'$, who will forward it to $C$ 
            \end{enumerate}

            When $b = 0$, the transcript $\tau$ sent by $C$ is taken from $\mathrm{Sim}(pk)$, otherwise it is taken from $P(pk,sk) \rightleftarrows V(pk)$. Therefore, $A$ can distinguish $H_i(\lambda)$ from $H_{i+1}(\lambda)$ through the $(i+1)$-th transcript, immediately concluding that $A'$ is a distinguisher for the HVZK game with non-negligible probability.
        \end{proof}

        \textbf{Claim 2}: for all PPT adversaries $A$ it holds that $\Pr[G_1(\lambda) = 1] \leq \mathrm{negl}(\lambda)$

        \begin{proof}{Proof of Claim 2.}
            This time we'll use a reduction from special soundness. By way of contradiction, suppose that there is an adversary $B = (B_1, B_2)$ that wins the $G_1(\lambda)$ game with probability at least $n^{-c}$ for some $c > 0$. We build a new adversary $B'$ defined as follows:
            \begin{enumerate}
                \item The challenger $C'$ sends $pk$ to $B'$, who will forward it to $B$
                \item $B$ makes $q$ transcript queries to $B'$. For each $j$-th query, $B'$ will answer with $\tau_j \in_R \mathrm{Sim}(pk)$. 
                \item Given the output $s = B_1(\tau_1, \ldots, \tau_q)$, play $\Sigma$ with $B_2(s,pk)$ instead of $P(pk,sk)$ and $\mathcal{B}_{\lambda, pk}$ instead of $V(pk)$, where $\mathcal{B}_{\lambda, pk}$ is the set of all second messages with $\omega(\log \lambda)$ bits that can be sent in $\Sigma$ using $pk$.
                \item $B$ returns $(\alpha^*, \beta_0^*, \gamma_0^*) \in_R B_2(s, pk) \rightleftarrows \mathcal{B}_{\lambda, pk}$ to $B'$
                \item The previous instructions between $B$ and $B'$ are repeated a second time (using the same $\tau_1, \ldots, \tau_q$ answers and the same first message $\alpha^*$), obtaining a second transcript $(\alpha^*, \beta_1^*, \gamma_1^*) \in_R B_2(s, pk) \rightleftarrows \mathcal{B}_{\lambda, pk}$
                \item $B'$ returns $(\alpha^*, \beta_0^*, \gamma_0^*), (\alpha^*, \beta_1^*, \gamma_1^*)$ to $C$
            \end{enumerate}

            The above reduction must be carefully analyzed. Let $Z \in \{0,1\}^*$ be the random variable representing the state of $B_2(s, pk)$ after sending the first message $\alpha^*$. Let also $\delta_z = \Pr[G_1(\lambda) = 1 \mid Z = z]$. We observe that:
            \[\frac{1}{n^c} \leq \Pr[G_1(\lambda) = 1] = \sum_{z \in \{0,1\}^*} \Pr[Z = z] \delta_z = \Exp[\delta_z]\]

            therefore $\Exp[\delta_z] \geq n^{-c}$. Let $G$ be the event such that $G = 1$ when $\beta_0^* \neq \beta_1^*$. When conditioning on $G = 1$, we get that:
            \[\begin{split}
            ...
            By Jensen's inequality we know that for any function $f$ on a r.v. $X$ it holds that $\Exp[f(X)] \geq f(\Exp[X])$. Therefore, we conclude that:
            \[\Pr[\mathrm{Game}_{\Sigma, B'}^{SS}(\lambda) = 1] \geq \Exp[\delta_z^2] - \frac{1}{2^{\omega(\log \lambda)}} \geq \Exp[\delta_z]^2 - \frac{1}{2^{\omega(\log \lambda)}} \geq \frac{1}{n^{2c-1}}\]
            
            and thus that $B'$ can win the SS game with non-negligible probability.
        \end{proof}
    \end{proof}

    \begin{framedcor}{}
        Schnorr's protocol has passive security.
    \end{framedcor}

    \section{The Fiat-Shamir transform}

    From the very definition of identification schemes, it's easy to see that they are closely related to the concept of digital signature. In particular, IDSs can be used to construct \textit{zero-knowledge digital signatures}. The first construction was given by Fiat and Shamir, without a proof of its security (later prove by Pointcheval and Stern). The idea is to construct an UFCMA-secure signature scheme starting from a non-trivial passively secure $\Sigma$-protocol (e.g. Schnorr's protocol).

    \begin{frameddefn}{Fiat-Shamir transform}
    ...
    \begin{framedthm}{}
        Let $\Sigma = (\mathrm{KGen}, P, V)$ be a $\Sigma$-protocol and let $H$ be an hash function. If $\Sigma$ is non-trivial and passively secure, the Fiat-Shamir transform $\Pi_{\Sigma, H}$ is UFCMA-secure in the ROM.
    \end{framedthm}

    \begin{proof}
        By way of contradiction, suppose that there is an adversary $A^H$ breaking UFCMA-security of $\Pi_{\Sigma, H}$ in the ROM. Without loss of generality, we assume that:
        \begin{itemize}
            \item $A^H$ never repeats queries and it makes a RO query before each signature query
            \item $A^H$ never queries $(\alpha, m)$ to the RO after receiving a signature $\sigma$ (this comes in handy since each signature query defines $\sigma = (\alpha,m)$ and $\beta = H(\alpha, m)$)
        \end{itemize}

        Let $\mathcal{B}_{\lambda, pk}$ is the set of all second messages with $\omega(\log \lambda)$ bits that can be sent in $\Sigma$ using $pk$.

        We build a new adversary $A$ for the ID-game of $\Sigma$, defined as follows:
        \begin{enumerate}
            \item $A$ will substitute the oracle for $H$, answering $A^H$ RO queries.
            \item The challenger $C$ sends $pk$ to $A$, who then forwards it to $A^H$
            \item $A^H$ tells $A$ that they will make $q = \poly(\lambda)$ RO queries. 
            \item For each $i \in [q]$, $A$ makes a transcript query to $C$, who then replies with $\tau_i = (\alpha_i, \beta_i, \gamma_i)$
            \item $A$ samples $j \in_R [q]$ and sets $H'$ as the function such that $H'(\alpha_i, m_i) = \beta_i$ when $i \neq j$ and $H'(\alpha_i m_i) = \beta^{\#}$ when $i = j$, where $\beta^{\#}$ is obtained by playing the first two messages of $\Sigma$ with $C$ ($A$ sends $\alpha_i$, $C$ returns $\beta^{\#}$ and the game gets paused)
            \item $A^H$ starts the RO queries. For each query $(\alpha_i, m_i)$, $A$ will answer with $\beta_i = H'(\alpha_i, m_i)$. One of these RO queries will be for the future challenge message $m^*$, meaning that $(\alpha^*, m^*) = (\alpha_{i^*}, m_{i^*})$ for some $i^* \in [q]$, but $A$ doesn't know which one 
            \item $A^H$ starts the sign queries. For each previously queried $m_i$ such that $m_i \neq m^*$, $A^H$ queries the signature of $m_i$ to $A$, which replies with $(\alpha_i, \gamma_i)$ if $i \neq j$ and with ABORT when $i = j$.
            \item After receiving the ABORT message, $A^H$ sends the challenge signature $(\alpha^*, \gamma^*)$ to $A$. $A$ then checks if $V(pk, \alpha^*, \beta^{\#}, \gamma^*) = 1$. If true, $A$ ends the game with $C$ sending the third message $\gamma^*$, otherwise $A$ sends a random final message.
        \end{enumerate}

        When the final check succeeds, the reduction is perfect since $\beta^{\#}$ guaranteed to be a valid pick. When $j = i^*$, this final check will always succeed (but it could also succeed in other cases), which happens with probability $1/q$.
    \end{proof}

    \begin{framedcor}{}
        The Fiat-Shamir transform $\Pi_{\Sigma_{\text{Schnorr}}, H}$ is UFCMA-secure in the ROM
    \end{framedcor}

    \chapter{Post-quantum cryptography}

    \section{Lattices and post-quantum hardness}
    ...
    \begin{frameddefn}{Lattice}
        Given a basis $B = \{b_1, \ldots, b_n\}$ with $b_i \in \R^n$, we define the lattice $\mathcal{L}(B)$ as the set of all integer linear combinations of $B$:
        \[\mathcal{L}(B) = \cbk{\sum_{i = 1}^{n} z_i b_i \mid z_1, \ldots, z_n \in \Z}\]
    \end{frameddefn}
    ...
    There are many problems involving the metric $\lambda_i(\mathcal{L})$ that are believed to be hard for post-quantum cryptography:
    \begin{itemize}
        \item \textit{Shortest Vector Problem (SVP)}: given a basis $B$ and a function $\gamma$, find a vector $v \in \mathcal{L}(B)-\{\vec 0\}$ such that $\norm{v} = \lambda_1(\mathcal{L}(B))$ or $\norm{v} \leq \gamma(n) \lambda_1(\mathcal{L}(B))$
        \item \textit{Shortest Independent Vectors Problem (SVP)}: given a basis $B$ and a function $\gamma$, find $n$ vectors $v_1, \ldots, v_n \in \mathcal{L}(B)-\{\vec 0\}$ such that $\norm{v_i} \leq \gamma(n) \lambda_n(\mathcal{L}(B))$ for all $i \in [n]$
        \item \textit{Gap of Shortest Vector Problem (SVP)}: given a basis $B$ and a function $\gamma$, determine if $\lambda_1(\mathcal{L}(B)) \leq 1$ or $\lambda_1(\mathcal{L}(B)) > \gamma(n)$.
    \end{itemize}

    In modern days, an equivalent problem is used for post-quantum cryptographic assumptions: the \textbf{Shortest Integer Solution (SIS)} problem. This problem is easier to reason with for modern crypto, due to its use of modular arithmetic. Given $m$ random vectors $a_i \in \Z_q^n$ forming a matrix $A \in \Z_q^{m \times n}$, the problem $\mathrm{SIS}_{n,q,\beta,m}$ asks to find a vector $z \in \Z_q^m-\{\vec 0\}$ with norm $\norm{z} = \beta$ such that $Az = \vec 0$.
    ...
    \section{Building crypto-tools through lattices}

    We introduce another problem believed to be hard for quantum computers, the \textbf{learning with errors (LWE)} problem. The problem is based on the idea of representing secret information as a set of equations with errors. In other words, LWE is a way to hide the value of a secret by introducing noise to it.
    
    Given a vector $\vec s \in \Z_q^n$ and an error distribution $\mathcal{X}$ over $\Z_q$, we define the LWE distribution $A_{\vec s,\mathcal{X}}$ as the distribution over $\Z_q^n \times \Z_q$ obtained by sampling a vector $\vec a \in_R \Z_q^n$ and an error $e \in_R \mathcal{X}$ and then outputting $(\vec a, b)$, where $b = \abk{\vec s,\vec a} +e \pmod q$. Given $m$ samples $(\vec a_i, \vec b_i) \in_R A_{\vec s,\mathcal{X}}$ for some unknown $\vec s^* \in \Z_q^{n}$, the $\mathrm{SEARCH-LWE}_{n,q,\mathcal{X},m}$ problem asks us to find $\vec s^*$ -- in other words, we're asked to find the distribution from which the $m$ samples have been extracted.

    We give some remarks on the SEARCH-LWE problem:
    \begin{itemize}
    ...
    \begin{framedass}{LWE assumption}
        The LWE assumption states that:
        \[(A_{\vec s, \mathcal{X}}) \approx_c (\Z_q^{m \times (n+1)})\]
    \end{framedass}

    The decisional version of the LWE problem can be used to construct a post-quantum PKE scheme, namely \textbf{Regev's PKE}.

    \begin{framedalgo}{Regev's PKE scheme}
        Given a vector $\vec s \in \Z_q^n$ and a value $m$ (with $m \approx (n+1) \log q$), the Regev PKE scheme $\Pi_{\text{Regev}} = (\mathrm{KGen}, \mathrm{Enc}, \mathrm{Dec})$ is defined as follows:
        \begin{itemize}
            \item $\mathrm{KGen}(1^\lambda) = (pk, sk)$ where $pk = A = \smat{\overline{A} & \vec b^T}^T$ and $sk = \vec s$, with each column of $A$ corresponding to a sample $(\overline{a_i}, b_i)$ with $b_i = \abk{\vec s, \overline{a_i}} + e_i \pmod q$
            \item $\mathrm{Enc}(pk, \mu) = \vec c$ where $\mu \in \{0,1\}$ and $\vec c = (pk) \vec r + \smat{\vec 0 & \mu \floor{q/2}}^T$, for some $\vec r \in_R \{0,1\}^m$ and where $\vec 0 \in \Z_q^n$
            \item $\mathrm{Dec}(pk, \vec c) = 1$ when $\smat{-\vec s, 1}^T \vec c \approx \mu \floor{q/2}$ and 0 otherwise
        \end{itemize}
    \end{framedalgo}

    To show the correctness of the above scheme, we observe that:
    \[\smat{- \vec s, 1}^T A = - \vec s \overline{A}+\overline{b}^T = \vec e^T= \vec 0 \pmod q\]

    Therefore, if $\vec c$ is a correct ciphertext we have that:
    \[\smat{- \vec s, 1}^T \vec c = \smat{- \vec s, 1}^T A \vec r + \mu \floor{\frac{q}{2}} = \vec e^T \vec r +  \mu \floor{\frac{q}{2}}\]

    which is almost equal to $m \floor{\frac{q}{2}}$ as long as $\abk{\vec e, \vec r}$ is less than $q/4$

    \begin{framedthm}{}
        Regev's PKE scheme is CPA-secure under the LWE assumption.
    \end{framedthm}

    \begin{proof}{Proof (Sketch).}
        The proof is subdivided in two steps. The first step is immediate: we  switch the matrix $A = \smat{\overline{A} & b}$ in the encryption function with a truly random matrix $M$ chosen over $\Z_q^{(n+1) \times m}$ -- easily obtained through the LWE assumption.

        The second step, instead, requires more discussion. Consider the sample $\vec r \in_R \{0,1\}^m$ chosen during the encryption process. We can use the left-over hash lemma to show that $M \vec{r}$ is statistically indistinguishable from a random vector as long as $\vec r$ has enough min-entropy -- in particular $m \approx (n+1) \log q$ suffices. In other words, the hash function given by $M \vec r$ is universal and thus also a seeded extractor, concluding that $\mu$ is information-theoretically hidden.
    \end{proof}

    Once the scheme has been proven to be secure for one bit, we can easily extend it to multiple bits: trivially, we individually encrypt each bit of the original message $\mu = (\mu_1, \ldots, \mu_\ell)$, obtaining the ciphertext $c = (c_1, \ldots, c_\ell)$ where each $c_i$ has size $n+1$.

    Since PKE encryption is possible even in the post-quantum world, the next natural objective would be to prove if secure signatures can also exist in this world. For classic computation, we discussed two main ideas: signatures built through trapdoors and signatures built through the Fiat-Shamir transform. 

    We start by discussing a \curlyquotes{quantum-proof} version of the Fiat-Shamir transform. The first immediate idea is to use the LWE assumption to build a $\Sigma$-protocol, such as the following protocol $\Sigma = (\mathrm{KGen}, P, V)$:
    \begin{itemize}
        \item $\mathrm{KGen}(1^\lambda) = (pk, sk)$ where $sk = \vec x \in_R \{0,1\}^m$ and $pk = (A, \vec u)$, with $A \in \Z_q^{m \times n}$ and $\vec u \in \Z_q^n$ chosen such that $A \vec x = \vec u$
        \item The transcripts are given by:
        \begin{enumerate}
    ...
    \begin{itemize}
        \item \textit{Version 2} - The transcripts are now given by:
        \begin{enumerate}
    ...
    Indeed, this fixes our issues with SS security (still under the LWE assumption). However, this fix now shatters HVZK security! To see this, consider the case when $\beta = 1$. Then, we have that $\beta \vec x = \vec x$ and $\vec \gamma = \vec x + \vec y$. Therefore, since $\vec y \in \{0,1\}^m$, if the $i$-th entry of $\vec \gamma$ is $\vec \gamma_i = 0$ this can happen only if $\vec x_i = 0$ and $\vec y_i = 0$. Similarly, if $\vec \gamma_i = 2$ then $\vec x_1 = 1$ and $\vec y_i = 1$. This concludes that information about the secret key gets leaked by transcripts even when the verifier is honest.

    Nonetheless, a middle ground between the two ideas, intuitively, solves both issues. The idea is to force $\vec y$ and $\beta$ to be small, but \underline{not too small}. In particular, after sampling $\vec y$ from a chosen range of values, we abort the protocol if one of the components of $\vec \gamma$ doesn't fall in a more specific range of values. After aborting, the protocol is repeated from the start. This final version is known as \textbf{Lyubashevsky's $\Sigma$-protocol} and the technique used in it is known as \textit{rejection sampling}.

    \begin{framedalgo}{Lyubashevsky's $\Sigma$-protocol}
        The Lyubashevsky $\Sigma$-protocol $\Pi_{\text{Lyub}} = (\mathrm{KGen}, P, V)$ is defined as follows:
        \begin{itemize}
            \item $\mathrm{KGen}(1^\lambda) = (pk, sk)$ where $sk = \vec x \in_R \{0,1\}^m$ and $pk = (A, \vec u)$, with $A \in \Z_q^{m \times n}$ and $\vec u \in \Z_q^n$ chosen such that $A \vec x = \vec u$
            \item The transcripts are given by:
            \begin{enumerate}
    ...
    Basically, each iteration of the protocol has a constant probabilty of not aborting that grows exponentially with the number of iterations. For security reasons, the protocol must be repeated for at least 128 times. 

    \begin{framedprop}{}
        Lyubashevsky's protocol has HVZK and SS under the LWE assumption.
    \end{framedprop}

    \begin{framedcor}{Fiat-Shamir with Aborts}
        The Fiat-Shamir transform constructed through Lyubashevsky's protocol is UFCMA-secure in the ROM under the LWE assumption.
    \end{framedcor}


    What about secure signatures? Similarly to public-key encryption, it suffices to show that basic tools can be constructed through quantum-safe assumptions. In particular, we'll focus on \textbf{lattice trapdoors}, i.e. trapdoors built on the LWE/SIS assumptions.

    We already know the drill: in the ROM, trapdoors can be used as an easy way to get signature schemes. For instance, consider the following signature scheme $\Pi = (\mathrm{Sign}, \mathrm{Vrfy})$ such that:
    \begin{itemize}
        \item $\mathrm{Sign}(sk, \mu)$ outputs a short vector $\vec x$ such that $A \vec x = H(\mu) = \vec u$ using a trapdoor $R$ associated with $A$

        \item $\mathrm{Vrfy}(pk, \mu, \sigma)$ checks if $\sigma$ is short and $A \sigma = H(\mu)$
    \end{itemize}
    
    where $H$ is a secure hash function. The proof of UFCMA-security in the ROM is very similar to the one seen for the Full-Domain Hash digital signature. We still require a way to generate trapdoors from lattices. The main idea is to use the easiness of the LWE and SIS problems for some particular structures.

    Consider the vector $\vec g = \smat{1 & 2 & \cdots & 2^{\ell-1}}^T \in \Z_{q}^{\ell}$ with $\ell = \ceil{\log q}$. A short $\vec x \in \Z^m$ such that $\abk{\vec g, \vec x} \equiv \vec u \pmod q$ is easily given by the vector $\vec x$ corresponding to the base 2 coefficients of $\mu$. In other words, given that $\mu = \sum_{i = 1}^{\ell} b_i \cdot 2^{i-1}$ for some coefficients $b_1, \ldots, b_\ell \in \{0,1\}$, if we set $\vec x = \smat{b_1 & b_2 & \ldots & b_\ell}$ the condition is satisfied. This gives a solution for the SIS problem.

    For simplicity, assume that $q = 2^\ell$. Consider a vector $\vec b$ such that $\vec b \approx s \cdot \vec g \pmod q$, with $s \in Z_q$. Since the $\ell$-th entry of $\vec b$ is such that $\vec b_\ell \approx s \cdot 2^{\ell-1} = s_\ell \cdot \frac{q}{2}$ -- with $s_\ell$ being the most significant bit of $s$ -- we have that $s_\ell \approx \frac{2\vec b_\ell}{q}$. The values of $s_{\ell-1}, \ldots, s_1$ can be found accordingly.

    This idea can also be extended to multi-dimensional settings. Consider the matrix $G = I_n \otimes \vec g^T$, where $\otimes$ is the Kronecker product operator:
    \[G = \smat{
        1 \cdot \vec g^T & 0 \cdot \vec g^T & \ldots & 0 \cdot \vec g^T \\
        0 \cdot \vec g^T & 1 \cdot \vec g^T & \ldots & 0 \cdot \vec g^T \\
        \vdots & \vdots & \ddots & \vdots\\
        0 \cdot \vec g^T & 0 \cdot \vec g^T & \ldots & 1 \cdot \vec g^T \\
    } = \smat{
        1 & 2 & \cdots & 2^{\ell-1} & 0 & 0 & \cdots & 0 & \cdots & 0 \\
        0 & 0 & \cdots & 0 & 1 & 2 & \cdots & 2^{\ell-1} & \cdots & 0 \\
        \vdots & \vdots & \ddots & \vdots & \vdots & \vdots & \ddots & \vdots & \ddots & \vdots \\
        0 & 0 & \cdots & 0 & 0 & 0 & \cdots & 0 & \cdots & 2^{\ell-1}
    }\]

    It's easy to see that for any invertible matrix $H$ the LWE (or SIS) problem is still easy with respect to $HG \in \Z_{q}^{n \times n\ell}$ because $(HG) \vec x = \vec u$ if and only if $G \vec x = H^{-1} \vec u$.

    Therefore, given a matrix $A \in \Z_q^{n \times m}$, any short matrix $R \in \Z_q^{m \times n\ell}$ such that $AR \equiv HG \pmod q$, for some invertible $H$, is effectively a trapdoor for $A$. Many techniques are known to generate matrices $A,R$ and $H$ related in this way, but we won't dive into them.
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[->,>=stealth,shorten >=1pt,auto,node distance=1cm,thick,main node/.style={scale=0.8,circle,draw,font=\sffamily\normalsize}]
            \node[] (1) []{Alice};
            \node[draw, rectangle, minimum width=40] (2) [right of = 1, xshift = 75]{$\mathrm{Enc}(mpk,\mathrm{ID},m)$};
            \node[] (3) [right of = 2, xshift = 75]{Bob};
            \node[] (4) [above of = 3, yshift = 40]{Authority};

            \node[] (5) [below of = 2, yshift = -10]{$(mpk,msk)$};
            \node[] (6) [below of = 1, yshift = -10]{};
            \node[] (7) [below of = 3, yshift = -10]{};
            \node[draw, rectangle, minimum width=40] (8) [right of = 3, xshift = 60]{$\mathrm{Dec}(d_{\mathrm{ID}},c)$};
            \node[] (9) [right of = 8, xshift = 50]{};
            
            \path[every node/.style={font=\sffamily\small}]
                (1) edge node{$m$} (2)
                (2) edge node{$c$} (3)
                (3) edge[bend left] node{$msk, \mathrm{ID}$} (4)
                (4) edge[bend left] node{$d_{\mathrm{ID}}$}(3)
                (5) edge[-] (6.center)
                (5) edge[-] (7.center)
                (6.center) edge (1)
                (7.center) edge (3)
        ...
        \caption{Example of identity-based encryption. Alice's ID is publicly known by every user.}
    \end{figure}

    The way in which this type of encryption generalizes public-key encryption is kinda subtle. Both parties can ignore Alice's ID and just use the master keys. In particular, Bob can act as the third party authority that always generates $d_{\mathrm{ID}} = msk$.

    \begin{frameddefn}{Identity-based Encryption (IDE) scheme}
        We say that a quadruple $\Pi = (\mathrm{Setup}, \mathrm{KGen}, \mathrm{Enc}, \mathcal{Dec})$ is an identification scheme (IDS) when:
        \begin{itemize}
            \item $(mpk,msk) \in_R \mathrm{Setup}(1^\lambda)$, with $mpk,msk \in \{0,1\}^\lambda$
            \item $\mathrm{ID}$ is a public unique identifier associated with the encrypting party
            \item $d_{\mathrm{ID}} \in_R \mathrm{KGen}(msk, \mathrm{ID})$ is a private key generated by the central authority
            \item $\forall m \in \mathcal{M}$ it holds that $\mathrm{Dec}(d_{\mathrm{ID}}, \mathrm{Enc}(mpk, \mathrm{ID}, m)) = m$
        \end{itemize}
    \end{frameddefn}

    IDE schemes come with a new issue arising from their very own definition: the third party authority has the ability to generate all secret keys, thus it can decrypt everything! This is known as \textbf{key escrow}. Then why is this type of encryption interesting if it adds a new problem? The idea is that there is no additional overload required regarding certificates and PKI (since the entire encryption scheme basically already contains this idea). This allows us to reason about a complete system without wasting energy in thinking how different parts should be connected together.

    Let's now focus on security definitions. The CPA game of IDE schemes is very similar to that of PKE schemes, with the additional complexity given by the third party authority generating secret keys: the attacker can also make a polynomial amount of \textbf{key extraction queries} before and after sending the challenger ciphertext. This version of the CPA game is referred to as ID-CPA.
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[->,>=stealth,shorten >=1pt,auto,node distance=1cm,thick,main node/.style={scale=0.8,circle,draw,font=\sffamily\normalsize}]
            ...
            \path[every node/.style={font=\sffamily\small}]
                (2t) edge[swap, near start] node{$mpk$} (1t)
                (3) edge node{$\mathrm{ID}_i$} (4)
                (6) edge[swap] node{$d_{\mathrm{ID}_i}$} (5)
                (5) edge[bend left] node {$\poly(\lambda)$}(3)
                (7) edge[near start] node{$\mathrm{ID}^*, m_0^*, m_1^*$} (8)
                (10) edge[swap, near start] node{$c^* = \mathrm{Enc}_\Pi(mpk,\mathrm{ID}^*, m_b^*)$} (9)
                (3t) edge node{$\mathrm{ID}_i' \neq \mathrm{ID}^*$} (4t)
                (6t) edge[swap] node{$d_{\mathrm{ID}_i'}$} (5t)
                (5t) edge[bend left] node {$\poly(\lambda)$}(3t)
                (11) edge[near start] node{$b' \in \{0,1\}$} (12)
                ;
        \end{tikzpicture}

        \caption{Graphical representation of $\mathrm{Game}_{\Pi, A}^{\text{ID-CPA}}(\lambda, b)$ for identity-based encryption.}
    \end{figure}

    The next step is defining a secure IDE scheme. In particular, the scheme will satisfy \underline{Selective} ID-CPA security, a weaker version of ID-CPA security where $\mathrm{ID}^*$ is chosen by the attacker \textit{before} $mpk$ is sent by the challenger. Proving that Sel-ID-CPA is weaker than ID-CPA -- meaning that the latter implies the former, but the converse isn't true -- is left as an exercise. Our construction is based on \textbf{bilinear groups}.

    \begin{frameddefn}{Bilinear group}
        A bilinear group is a triple $(\G, \G_T, \hat e)$ such that:
        \begin{enumerate}
            \item $\G$ a cyclic group of order $q$ generated by $g$, referred to as the source group
            \item $\G_T$ a cyclic group of order $q$ generated by $\hat e(g,g)$, referred to as the target group
            \item $\hat e : \G \times \G \to \G_T$ is bilinear, meaning that  that $\forall g,h \in \G$ and $\forall a,b \in \Z_q$ it holds that $\hat e(g^a, h^b) = \hat e(g,h)^{ab}$
        \end{enumerate}
    \end{frameddefn}

    There are known bilinear groups based on elliptic curves with an efficiently computable bilinear function, but we wont dive into them. For our purposes, we can just assume that they exist. It's easy to see that the Decisional Diffie-Hellman (DDH) assumption doesn't hold in bilinear groups. Given $X = g^x$, $Y = g^y$ and $Z \in \{g^{xy}, g^z\}$, with $x,y,z \in_R \Z_q$, when $Z = g^{xy}$ it holds that:
    \[\hat e (g, Z) = \hat e (g, g^{xy}) = \hat e (g, g)^{xy} = \hat e(g^x, g^y) = \hat e (X, Y)\]

    Therefore, we can just check if $\hat e(g,Z) = \hat e (X,Y)$ in order to win the DDH game with non-negligible probability. Nonetheless, bilinear groups are believed to be sufficiently secure for the outputs of $\hat e$. In particular, the application of $\hat e$ onto three different elements of $\G$ is believed to be indistinguishable from a truly random element of $\G_T$. This is known as the \textbf{Decisional Bilinear Diffie-Hellman (DBDH)} assumption.

    \begin{framedass}{DBDH assumption}
        Given a quintuple $(\G,\G^T,g,q, \hat e)$ where $(\G, \G_T, \hat e)$ is a bilinear group, $g \in \G$ is a generator and $q$ is the order of $g$ in $\G$, the DBDH assumption states that:
        \[(\G,g,q,g^a,g^b,g^c, \hat e(g,g)^{abc}) \approx_c (\G,g,q,g^a,g^b,g^c, T)\]
        with $a,b,c \in_R \Z_q$ and $T \in_R \G_T$, for all PPT adversaries $A$ and for a chosen value $\lambda > 0$
    \end{framedass}

    This assumption can be used to construct a Selective ID-CPA secure IDE scheme known as the \textbf{Boneh-Franklin scheme}.

    \begin{framedalgo}{Boneh-Franklin IDE scheme}
        The Boneh-Franklin IDE scheme $\Pi_{\text{BF}} = (\mathrm{Setup}, \mathrm{KGen}, \mathrm{Enc}, \mathrm{Dec})$ is defined as follows:
        \begin{itemize}
            \item $\mathrm{Setup}(1^\lambda) = (mpk, msk)$ where $mpk = (\G, \G_T, g, q, g_1, g_2, h)$ and $msk = g_2^\alpha$, with $(\G,\G_T, g,q,\hat e)$ being a bilinear group, $h \in_R \G$, $g_2 \in_R \G$ and $g_1 = g^\alpha$ for some $\alpha \in_R \Z_q$ 
            \item $\mathrm{KGen}(msk, ID) = (d_0, d_1)$ where $d_0 = msk \cdot F(\mathrm{ID})^r$ and $d_1 = g^r$, with $r \in_R \Z_q$ and $F : \Z_q \to \G$ being a pre-chosen common function such that $F(\mathrm{ID}) = g_1^{\mathrm{ID}} \cdot h$
            \item $\mathrm{Enc}(mpk, ID, m) = (u,v,w)$ where $u = \hat e(g_1, g_2)^\gamma \cdot m$, $v = g^\gamma$ and $w = F(\mathrm{ID})^\gamma$, with $\gamma \in_R \Z_q$
            \item $\mathrm{Dec}((d_0, d_1), (u,v,w)) = \dfrac{u \cdot \hat e (d_1, w)}{\hat e (v, d_0)}$
        \end{itemize}
    \end{framedalgo}

    The correctness of the decryption process is given by:
    \[\begin{split}
        \frac{u \cdot \hat e (d_1, w)}{\hat e (v, d_0)} &= \hat e(g_1, g_2)^\gamma \cdot m \cdot \frac{\hat e (g^r, F(\mathrm{ID})^\gamma)}{\hat e (g^\gamma, g_2^\alpha \cdot F(\mathrm{ID})^r)} \\
        &=  \hat e(g^\alpha, g_2)^\gamma \cdot m \cdot \frac{\hat e (g, F(\mathrm{ID}))^{r\gamma}}{\hat e (g^\gamma, g_2^\alpha) \cdot \hat e(g^\gamma, F(\mathrm{ID})^r)} \\
        &=  \hat e(g, g_2)^{\gamma\alpha} \cdot m \cdot \frac{\hat e (g, F(\mathrm{ID}))^{r\gamma}}{\hat e (g, g_2)^{\gamma\alpha} \cdot \hat e(g, F(\mathrm{ID}))^{r\gamma}} \\
        &= m
    \end{split}\]


    \begin{framedthm}{}
        The Boneh-Franklin IDE scheme is Selective ID-CPA secure under the DBDH assumption.
    \end{framedthm}
    ...
    \begin{proof}
        ...
        \begin{enumerate}
            ...
            \item $A$ makes $q = \poly(\lambda)$ extraction queries to $A'$. For each $\mathrm{ID}_i \neq \mathrm{ID}^*$ queried by $A$, $A'$ answers with $d_{\mathrm{ID}_i} = (d_0^{(i)}, d_1^{(i)})$, where $d_0^{(i)} = Y^{-a/(\mathrm{ID}_i-\mathrm{ID}^*)} \cdot F(\mathrm{ID})^{r_i}$ and $d_1^{(i)} = \dfrac{g^{r_i}}{Y^{1/(\mathrm{ID}_i-\mathrm{ID}^*)}}$ for some $r_i \in_R \Z_q$
            \item $A'$ sends the challenge messages $m_0^*, m_1^*$ to $A'$, who then answers with $c^* = (u,v,w)$ such that $u = T \cdot m_b^*, v =Z$ and $w = Z^{a}$
            \item $A$ makes some more extraction queries in the same fashion as before.
        ...
        \[\begin{split}
            d_0^{(i)} &= Y^{-a/(\mathrm{ID}_i - \mathrm{ID}^*)} \cdot F(\mathrm{ID}_i)^{r_i} \\
            &= Y^{-a/(\mathrm{ID}_i - \mathrm{ID}^*)} \cdot (X^{\mathrm{ID}_i} \cdot h)^{r_i} \\
            &= Y^{-a/(\mathrm{ID}_i - \mathrm{ID}^*)} \cdot (X^{\mathrm{ID}_i-\mathrm{ID}^*} \cdot g^a)^{r_i} \cdot \frac{(X^{\mathrm{ID}_i - \mathrm{ID}^*} \cdot g^a)^{\beta/(\mathrm{ID}_i - \mathrm{ID}^*)}}{(X^{\mathrm{ID}_i-\mathrm{ID}^*} \cdot g^a)^{\beta/(\mathrm{ID}_i - \mathrm{ID}^*)}}\\
            &= g^{-a\beta/(\mathrm{ID}_i - \mathrm{ID}^*)} \cdot (X^{\mathrm{ID}_i-\mathrm{ID}^*} \cdot g^a)^{r_i} \cdot \frac{X^{\beta} \cdot g^{a\beta/(\mathrm{ID}_i - \mathrm{ID}^*)}}{(X^{\mathrm{ID}_i - \mathrm{ID}^*} \cdot g^a)^{\beta/(\mathrm{ID}_i - \mathrm{ID}^*)}}\\
            &= X^{\beta} \cdot (X^{\mathrm{ID}_i-\mathrm{ID}^*} \cdot g^a)^{\tilde{r_i}}\\
    ...
    Other than being an alternative to public-key infrastructures, IBE schemes can also be used to derive less complex secure types of cryptography. For instance, we can combine a ID-CPA-secure IBE with a UFCMA-secure digital signature to get a CCA-secure public-key encryption scheme. But wait, we already saw that this can be achieved through a CPA-secure PKE scheme instead of a CPA-secure IBE scheme\dots how can this be of our interest? Surprisingly, for IBE schemes Selective ID-CPA security is sufficient when combined with UFCMA-security.

    Consider an IBE scheme $\Pi = (\mathrm{Setup}, \mathrm{KGen}, \mathrm{Enc}, \mathrm{Dec})$ and a digital signature scheme $\Pi' = (\mathrm{KGen}', \mathrm{Sign}, \mathrm{Vrfy})$. We define a PKE scheme $\Pi'' = (\mathrm{KGen}'', \mathrm{Enc}'', \mathrm{Dec}'')$ where:
    \begin{itemize}
        \item $\mathrm{KGen}''(1^{\lambda}) = (ek, dk)$ with $(ek,dk) = (mpk, msk)$, where $(mpk, msk) \in_R \mathrm{Setup}(1^{\lambda})$
        \item $\mathrm{Enc}''(ek, m) = (c,vk,\sigma)$ with $c = \mathrm{Enc}(ek, vk, m)$ and $\sigma = \mathrm{Sign}(sk, c)$, where $(vk, sk) \in_R \mathrm{KGen}'(1^{\lambda})$
        \item $\mathrm{Dec}''(dk, (c,vk,\sigma)) = \bot$ when $\mathrm{Vrfy}(vk, c, \sigma) = 1$, otherwise $\mathrm{Dec}''(dk, (c,vk,\sigma)) = \mathrm{Dec}(mpk, d_{vk}, c)$, with $d_{vk} \in_R \mathrm{KGen}(msk, vk)$
    \end{itemize}
    
    This PKE scheme can be proven to be CCA-secure when the initial schemes are Selective ID-CPA secure and UFCMA-secure. 

    \begin{framedthm}{}
        Let $\Pi$ be a Selective ID-CPA secure scheme and let $\Pi'$ be a UFCMA-secure scheme. Then, the PKE scheme $\Pi''$ of the above construction is CCA-secure.
    \end{framedthm}

    \begin{proof}
        Omitted (very similar to the case of SKE schemes.)
    \end{proof}

    Moreover, secure digital signatures themselves are implied by IBE schemes -- again, under some constraints. We start by observing that Selective ID-CPA sc

    \begin{framedthm}{}
        Any IBE scheme $\Pi$ that is $(t,q,\varepsilon)$-Selective ID-CPA secure is also $(t,q,N\varepsilon)$ ID-CPA secure, with $N = \abs{\mathcal{ID}}$ and $\mathcal{ID}$ being the set of all possible IDs of $\Pi$

        \textit{Note}: $(s,p,\delta)$-security means that any PPT adversary running in time $s$ with $p$ extraction queries has an advantage of at most $\delta$
    \end{framedthm}

    \begin{proof}
        Omitted (the two games are identical when the extraction queries never ask for the initially selected ID)
    \end{proof}

    As long as $N = \poly(\lambda)$ and $\varepsilon = \mathrm{negl}(\lambda)$, the above theorem implies that Selective ID-CPA secure IBE schemes are also ID-CPA secure (notice that the converse is always true, making the two concepts equivalent under these constraints).

    Now, consider the following new notion of security dubbed \textbf{One-way ID-CPA security (OF-ID-CPA)}, describing the idea of an encryption function being hard to invert.
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[->,>=stealth,shorten >=1pt,auto,node distance=1cm,thick,main node/.style={scale=0.8,circle,draw,font=\sffamily\normalsize}]
            ...
            \path[every node/.style={font=\sffamily\small}]
                ...
                (6) edge[swap] node{$d_{\mathrm{ID}_i}$} (5)
                (5) edge[bend left] node {$\poly(\lambda)$}(3)
                (7) edge[near start] node{$\mathrm{ID}^*$} (8)
                (10) edge[swap, near start] node{$c^* = \mathrm{Enc}_\Pi(mpk,\mathrm{ID}^*, \mu^*)$ with $\mu^* \in_R \mathcal{M}$} (9)
                (3t) edge node{$\mathrm{ID}_i' \neq \mathrm{ID}^*$} (4t)
                (6t) edge[swap] node{$d_{\mathrm{ID}_i'}$} (5t)
                (5t) edge[bend left] node {$\poly(\lambda)$}(3t)
                (11) edge[near start] node{$b' \in \{0,1\}$} (12)
        ...
        \caption{Graphical representation of $\mathrm{Game}_{\Pi, A}^{\text{OF-ID-CPA}}(\lambda, b)$ for identity-based encryption.}
    \end{figure}

    \begin{framedprop}{}
        Let $\Pi$ be a ID-CPA secure scheme with $\frac{1}{\abs{\mathcal{M}}} = \mathrm{negl}(\lambda)$. Then, $\Pi$ is also OW-ID-CPA secure.
    \end{framedprop}

    \begin{proof}
        Omitted (a statistical difference argument suffices).
    \end{proof}

    This new security concept can be used to get a secure digital signature. Again, let $\Pi = (\mathrm{Setup}, \mathrm{KGen}, \mathrm{Enc}, \mathrm{Dec})$ be an IDE scheme. We define a digital signature $\Pi^* = (\mathrm{KGen}^*, \mathrm{Sign}^*, \mathrm{Vrfy}^*)$, where:
    \begin{itemize}
        \item $\mathrm{KGen}^*(1^\lambda) = (vk, sk)$ with $(vk,sk) = (mpk, msk)$, where $(mpk, msk) \in_R \mathrm{Setup}(1^{\lambda})$
        \item $\mathrm{Sign}^*(sk, m) = d_m$ with $d_m \in_R \mathrm{KGen}(msk, m)$
        \item $\mathrm{Vrfy}^*(vk, m, \sigma) = \1[\mathrm{Dec}(d_{m}, c) = \mu]$ with $\mu \in_R \mathcal{M}$ and $c \in_R \mathrm{Enc}(mpk, m, \mu)$
    \end{itemize}

    \begin{framedthm}{}
        Let $\Pi$ be a OF-ID-CPA secure scheme. Then, the digital signature scheme $\Pi^*$ of the above construction is UFCMA-secure.
    \end{framedthm}

    \begin{proof}
        Omitted (immediate reduction).
    \end{proof}
