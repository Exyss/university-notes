...
\begin{document}
    ...
    \begin{proof}
        ...
        \[\begin{split}
            ...
            &= Y^{-a/(\mathrm{ID}_i - \mathrm{ID}^*)} \cdot (X^{\mathrm{ID}_i} \cdot X^{-\mathrm{ID}_i} \cdot g^a)^{r_i} \\
    ...
    When the message space of the IBE is large enough, the above digital signature is OW-ID-CPA secure.
    ...
    When the message space of the initial IBE is large enough, the above digital signature is OW-ID-CPA secure.
    ...
    All the above theorems can be combined to get a CCA-secure PKE scheme starting from a Selective CPA-secure IBE with $\frac{1}{\abs{\mathcal{M}}} = \mathrm{negl}(\lambda)$ and $\abs{\mathcal{ID}} = \poly(\lambda)$.
\end{document}
