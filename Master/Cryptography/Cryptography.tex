...
\begin{document}
    ...
    \begin{proof}
        ...
        \begin{proof}[Proof of Claim 1.]
            ...
            \begin{enumerate}
                ...
                \item Each encryption query $m_i$ from $A'$ to $A_1$ is forwarded to $C_1$, which replies with the chiphertext $c_i$
    ...
    \section{Block chiphers and Feistel networks}
    ...
    In practical applications, PRPs are referred to as \textbf{block chiphers}. The two most common block chiphers are known as \textit{Data Encryption Standard (DES)} and \textit{Advances Encryption Standard (AES)}. We remark that neither of these block chiphers can be really proven to be secure under standard assumptions (recall that we don't know if PRFs really do exist in practice). However, their design is high inspired by standard results in provable security. The DES block chipher is based on the theoretical notion of \textbf{Feistel networks}, a way to construct a PRP family from any PRF family.
