...
\begin{document}
    ...
    \begin{enumerate}
        \item The challenger sends a seed $s \in_R U_\lambda$
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[->,>=stealth,shorten >=1pt,auto,node distance=1cm,thick,main node/.style={scale=0.8,circle,draw,font=\sffamily\normalsize}]
            ...
            \path[every node/.style={font=\sffamily\small}]
                (6) edge[swap, near start] node{$s \in_R U_\lambda$} (5)
    ...
    \begin{proof}
        The idea is to basically reduce a collision for $\mathcal{H}_{MD}$ to a collision for $\mathcal{H}$. Fix a seed $s \in_R U_\lambda$. Assume that $x = (x_1, \ldots, x_d)$ and $x' = (x'_1, \ldots, x'_d)$ are two inputs such that $x \neq x'$ and $H_s(x) = H_s(x')$.
    ...
    Hash functions are also used to build MACs. The HMAC scheme uses $\mathrm{Tag}(K, m) = H(K \mid\mid m)$, with $H$ being hash function. If $H(\cdot) \in \mathcal{R}(\ell(n) \to n)$, the HMAC scheme is secure. If $H$ is constructed through the Merkle-Damg\aa rd transform, the scheme can be broken. The study of cryptosystems under the usage of an ideal cipher is called \textbf{Random Oracle Model}.
    ...
    \begin{proof}
        ...
        \begin{enumerate}
            ...
            \item $A^{\text{DL}}_{\G}$ samples $i \in_R U_n$ and sets $g_i = y$
        ...
        Observe that the probability of this assumption to be true is at least $1/n$ since $i \in_R U_n$ and $(x_1, \ldots, x_n) \neq (x_1', \ldots, x_n')$. Therefore, we conclude that:
    ...
    \begin{framedalgo}{ElGamal PKE scheme}
        Given a triple $(\G,g,q)$ where $\G$ is a cyclig group, $g \in \G$ is a generator and $q$ is the order of $g$ in $\G$, the ElGamal PKE scheme $\Pi = (\mathrm{KGen}, \mathrm{Enc}, \mathcal{Dec})$ is defined as follows:
        \begin{itemize}
