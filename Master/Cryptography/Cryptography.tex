...
\begin{document}
    ...
    \begin{proof}
        ...
        \begin{proof}[Proof of Claim 1.]
            ...
            \begin{enumerate}
                \item The adversary $A'$ starts both with the challenger $C$ and the other adversary $A$
    ...
    \section{Computationally secure MACs}
    ...
    \begin{framedthm}{UFCMA-security through PRFs}
        If $\mathcal{F} = \{F_K : \{0,1\}^n \to \{0,1\}^\lambda\}_{K \in \{0,1\}^\lambda}$ is a PRF family, $\mathrm{Tag}(K, m) = F_K(m)$ is UFCMA-secure for FIL.
    \end{framedthm}
    ...
    \begin{framedthm}{Composition of PRFs with AU hash families}
    ...
    \begin{proof}
        ...
        Therefore, it remains to show that $H(\lambda) \approx G(\lambda,1)$. Let $m_1, \ldots, m_t$ be the encryption queries made by the adversary, where $t = \poly(\lambda)$. Let $B$ be the bad event such that $B = 1$ if $\exists i,j \in [t]$ with $i \neq j$ for which $h_s(m_i) = h_j(m_j)$. By conditioning on $B = 0$, no collisions occurr, thus:
    ...
    \begin{enumerate}
        ...
        \end{split}\]
        which is true with probability $(d-1)2^{-n}$. This concludes that $\mathcal{H}$ is an almost universal hash family.
    \end{enumerate}
