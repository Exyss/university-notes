...
\end{frameddefn}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[scale=1.2]{images/perceptron.eps} %
    ...
    \begin{enumerate}
        \item First, compute the derivative of  a variant\footnote{
            since the sign function is not differentiable, we consider a variant of the error function without the filtering term.
        } of $E(w)$ with respect to each component $w_j$ of $ w$
        \[\frac{\partial  E}{\partial  w_j} = \sum_{i = 1}^N (t_i - w^Tx_i)(-x_{j,i})\]
    
        \item Update each component $w_j$ by setting $ w_j := w_j + \Delta w_j$, where:
        \[\Delta w_j = - \eta \frac{\partial  E}{\partial  w_j} = \eta \sum_{i = 1}^N (t_i - w^Tx_i)x_{j,i}\]
    ...
    After learning the optimal $ w$, the predicted class of a new instance $x \in \R^d-\R^d_D$ is -- in both tresholded and untresholded perceptrons -- given by $\mathrm{sign}({w}^T{x})$.
    ...
    We also notice that, since this doesn't affect the solution, we can also rescale all the points in such a way that the closest have unitary length. Hence, we consider the canonical representation where $t_i(w^Tx + w_0) \geq 1$ for all $i \in [N]$. In this setup, the optimal solution is given by:
