...
\begin{document}
    ...
    \begin{proof}
        Before proving the result, we discuss its non-trivialness: even thought $S_i = \sum_{j = 1}^{+\infty} S_i^{(j)}$, we cannot directly establish the result since $V_i$ is a random variable -- which may be dependent on other variables. In fact, an independence assumption will be made throughout the proof. First, we observe that:
    ...
    \[\Exp[N^{\text{queue}}] = \Exp[N] - \Exp[N^{\text{server}}] = \frac{\rho}{1-\rho} - \rho = \frac{\rho^2}{1-\rho}\]
    ...
    \section{Kendall's notation and common devices}

    Up until now, we have strictly focused on devices formed of an infinite queue and server. Now, we'll discuss devices in a more general way. First of all, we introduce a \textit{notation} that will allow us to talk about devices in a more coincise way. In particular, we'll use \textbf{Kendall's notation}, that is the notation M/M/k/M$_Q$ where:
    \begin{itemize}
    ...
    \subsection{Analysis of M/M/1/M$_Q$ devices}
    
    \label{mmm1}
    The \curlyquotes{standard} device that we have considered up until now is a device with memoryless arrivals, memoryless departures, one server and an infinte queue. Using Kendall's notation, this device is described as M/M/1. 

    \begin{figure}[H]
        \centering
        \includegraphics[scale=0.85]{images/mm1.png}
        \caption{The M/M/1 queueing system.}
    \end{figure}
    ...
    \begin{figure}[H]
        ...
        \caption{The CTMC of a M/M/1 queueing system.}
    \end{figure}

    As always, we start by computing the limiting distribution. Using the barrier technique, we obtain the following stationary equations:
    \[\soe{rlcl}{
        \pi_{i} &= \frac{\lambda}{\mu} \pi_{i-1} & \qquad & \forall i \in \N-\{0\} \\
        \sum\limits_{i = 0}^{+\infty} \pi_i &= 1 
    }\]

    By unrolling the recursive equation we get that:
    \[\pi_i = \rbk{\frac{\lambda}{\mu}}^i \pi_0 \qquad \forall i \in \N\]
    ...
    which concludes that $\pi_0 = 1-\rho$. Therefore, it holds that $\pi_i = \rho^i (1-\rho)$ for all $i \in \N$. After computing the limiting distribution, we can focus on computing other performance measures.
    
    First of all, we observe that $\rho$ is still equal to the utilization of the device. In fact, since $\pi_0$ equals to the probability of the device being idle due to no jobs being inside of it, it should make sense that $\pi_0 = 1-\rho$. Moreover, we also observe that this implies that $\rho < 1$ must hold in order to have $\pi_0 > 0$, which also implies that $\lambda < \mu$ must hold, deriving the \textbf{stability condition}.

    Moving on, we focus on performance metrics defined through expected values. Up until now, we have treated these values as nothing more than \curlyquotes{a definition} in order to compute them. Now, we can finally use our new tools to compute them in the proper way. Let's start with $\Exp[N]$. By definition, we have that:
    \[\Exp[N] = \sum_{i = 0}^{+\infty} i \Pr[N = i] = \sum_{i = 0}^{+\infty} i \pi_i\]
    since $\pi_i$ represents the probability of being in state $i$ when the system is in steady mode. Hence, we get that:
    \[\begin{split}
        \Exp[N] &= \sum_{i = 0}^{+\infty} i \rho^i (1-\rho) = \rho \sum_{i = 0}^{+\infty} i \rho^{i-1} (1-\rho) = \frac{\rho}{1-\rho} \\
    \end{split}\]
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[scale=0.75]{images/exp_n.png}
        \caption{The expected population in an M/M/1 system as the value of $\rho$ increases.}
    \end{figure}

    What about the expected population in the server, i.e. $\Exp[N^{\text{server}}]$. Using the formal definition of expected value we get that:
    \[\Exp[N^{\text{server}}] = 0 \cdot \Pr[N^{\text{server}} = 0] + 1 \cdot \Pr[N^{\text{server}} = 1] = 0 \cdot \pi_0 + 1 \cdot \rbk{\sum_{i = 1}^{+\infty} \pi_i} = 1 - \pi_0 = \rho\]
    ...
    which is exactly the \textbf{utilization law}! Furthermore, by applying Little's law we get that:
    \[\Exp[R] = \frac{\Exp[N]}{X} = \frac{\rho}{\lambda(1-\rho)} = \frac{1}{\rho(1-\rho)} = \frac{1}{\mu-\lambda}\]
    ...
    \[\Exp[W] = \Exp[R] - \Exp[S] = \frac{1}{\mu-\lambda} - \frac{1}{\mu} = \frac{\rho}{\mu-\lambda}\]


    Suppose now that the size of the queue is finite, meaning that we're considering a M/M/1/M$_Q$ device with Poisson arrivals of rate $\lambda$ and exponential service rate $\mu$. We observe that this device can have up to $M_Q+1$ jobs inside it: 1 in the server and M$_Q$ in the queue. Therefore, we can represent this device with the following CTMC.
    ...
    \begin{figure}[H]
        ...
        \resizebox{0.75\textwidth}{!}{
            \begin{tikzpicture}[->,>=stealth,shorten >=1pt,auto,node distance=2.5cm,thick,main node/.style={scale=0.6, circle,draw,font=\sffamily\normalsize}]
                ...
                \node[state, minimum size = 50] (0) []{$0$};
                \node[state, minimum size = 50] (1) [right of = 0]{$1$};
                \node[state, minimum size = 50] (2) [right of = 1]{$2$};
                \node[state, minimum size = 50] (3) [right of = 2]{$3$};
                \node[]      (4) [right of = 3]{$\cdots$};
                \node[state, minimum size = 50] (5) [right of = 4]{$M_Q+1$};
        ...
        \caption{The CTMC of a M/M/1/M$_Q$ queueing system.}
    \end{figure}
    ...
    \[\soe{rlcl}{
        \pi_{i} &= \frac{\lambda}{\mu} \pi_{i-1} & \qquad & \forall i \in \{1, \ldots, m\} \\
        \sum\limits_{i = 0}^{m} \pi_i &= 1 
    }\]

    By unrolling the recursive equation we get that:
    \[\pi_i = \rbk{\frac{\lambda}{\mu}}^i \pi_0 \qquad \forall i \in \{0, \ldots, m\}\]
    ...
    Is the utilization of the device still equal to $\rho$? The answer is \underline{no}. To distinguish it from the value $\rho$, we'll denote the utilization as $U$, which is given by:
    ...
    \[\begin{split}
        \Exp[N] &= \sum_{i = 0}^{m} i \pi_i \\
        &= \sum_{i = 0}^{m} i \frac{\rho^i(1-\rho)}{1-\rho^{m+1}} \\
        &= \frac{\rho}{1-\rho^{m+1}} \sum_{i = 0}^{m} i \rho^{i-1} (1-\rho)\\
        &= \frac{\rho}{(1-\rho^{m+1})(1-\rho)}
    \end{split}\]

    To compute the throughput of a device with finite queue, we must consider that packets may indeed be \textit{dropped}: if the buffer is full and another job arrives, it must be dropped. Let $\lambda_{\text{drop}}$ be the drop rate, that being the value such that $X = \lambda - \lambda_{\text{drop}}$. We observe that, for a generic M/M/1/M$_Q$ system, packets get dropped only when the queue reaches its full capacity, i.e. when the population in the system reaches $M_Q + 1$. In other words, we have that:
    \[\lambda_{\text{drop}} = \lambda \cdot \Pr[\text{drop a packet}] = \lambda \pi_{M_Q+1} = \frac{\lambda \rho^m(1-\rho)}{1-\rho^{m+1}}\]

    Other performance metrics can be computed in the same way by applying Little's law. In particular, we observe that as $m \to +\infty$ every performance metric of the M/M/1/M$_Q$ device tends to become equal to the one of the M/M/1/$\infty$ device.

    \subsection{Analysis of M/M/k/M$_Q$ devices}

    After showing that CTMC analysis is sound for M/M/1/M$_Q$ devices we move to more complex structures, that being M/M/k/M$_Q$ devices, also called \textit{multi-server systems}. We assume to be working with Poisson arrivals of rate $\lambda$ and that every server of the device shares the same exponential service rate $\mu$.

    \begin{figure}[H]
        \centering
        \includegraphics[scale=0.75]{images/mmk.png}
        \caption{The M/M/k/M$_Q$ queueing system.}
    \end{figure}
    ...
    However, this commodity comes at a price: considering the servers as a unique box implies that more jobs may depart from the system based on the number of servers that are working. For instance, if only one job is in the system we know that it can depart with rate $\mu$ since only one server of the ensable is working, but if two jobs are in the system then they can depart with rate $2\mu$ since two servers are working, and so on. When all $k$ servers are busy, meaning that we're in state $i \in \{k, \ldots, M_Q+k\}$, the total service rate is fixed to $k\mu$ since no more server can be working. Therefore, we get the following Markov chain.
    ...
    \begin{figure}[H]
        ...
        \resizebox{1\textwidth}{!}{
            \begin{tikzpicture}[->,>=stealth,shorten >=1pt,auto,node distance=2.5cm,thick,main node/.style={scale=0.8, circle,draw,font=\sffamily\normalsize}]
                ...
                \node[state, minimum size = 50] (4) [right of = 3]{$k$};
                \node[state, minimum size = 50] (5) [right of = 4]{$k+1$};
                \node[]      (6) [right of = 5]{$\cdots$};
                \node[state, minimum size = 50] (7) [right of = 6]{$M_Q+k$};
                ...
                \foreach \curr [
                    ...
                    \path[every node/.style={font=\sffamily\small}]
                        (7) edge[bend left] node{$k\mu$} (6)
                        (6) edge[bend left] node{$k\mu$} (5)
                        (5) edge[bend left] node{$k\mu$} (4)
                        (4) edge[bend left] node{$k\mu$} (3)
        ...
        \caption{The CTMC of a M/M/k/M$_Q$ queueing system.}
    \end{figure}

    To find the limiting distribution, we can still use the barrier technique, obtaining the following stationary equations:
    \[\soe{rlcl}{
        \pi_{i} &= \frac{\lambda}{i \mu} \pi_{i-1} & \qquad & \forall i \in \{1, \ldots, k-1\} \\
        \pi_{i} &= \frac{\lambda}{k\mu} \pi_{i-1} & \qquad & \forall i \in \{k, \ldots, m\} \\
        \sum\limits_{i = 0}^{m} \pi_i &= 1 
    }\]
    ...
    \[\begin{split}
        ...
        &= \sum_{i = 0}^{k-1} \dfrac{1}{i!} \rho^i \pi_0 + \sum_{i = k}^{m} \dfrac{1}{k! \cdot k^{i-k}} \rho^i \pi_0 \\
        &= \pi_0 \rbk{\sum_{i = 0}^{k-1} \dfrac{\rho^i}{i!} + \frac{1}{k!} \sum_{i = k}^{m} \dfrac{1}{k^{i-k}} \rho^i} \\
        &= \pi_0 \rbk{\sum_{i = 0}^{k-1} \dfrac{\rho^i}{i!} + \frac{1}{k! \rho^{k}} \sum_{h = 0}^{m-k} \rbk{\dfrac{\rho}{k}}^h} \\
        &= \pi_0 \rbk{\sum_{i = 0}^{k-1} \dfrac{\rho^i}{i!} + \frac{1-\theta^{m-k+1}}{k! \rho^{k} (1-\theta)}}\\
    \end{split}\]

    where $\theta = \rho / k$. Therefore, we have that:
    \[\pi_0 = \rbk{\sum_{i = 0}^{k-1} \dfrac{\rho^i}{i!} + \frac{1-\theta^{m-k+1}}{k! \rho^{k} (1-\theta)}}^{-1}\]

    Sadly, this cannot be expressed in a more coincise way -- actually it could, but it requires advanced calculus functions. After finding out the limited distribution, we can compute each performance metric proceeding as we did for M/M/1/M$_Q$ devices.

    \section{Systems with multiple devices}

    After analyzing systems composed of a single device with one or multiple server, we start to investigate networks with multiple devices. In particular, consider the \textit{tandem} of M/M/1/0 devices represented in the figure below.

    \begin{figure}[H]
        \centering
        \includegraphics[scale=0.75]{images/tandem.png}
        \caption{A tandem of M/M/1/0 devices.}
    \end{figure}
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[->,>=stealth,shorten >=1pt,auto,node distance=2.5cm,thick,main node/.style={scale=0.8, circle,draw,font=\sffamily\normalsize}]
            ...
            \node[state] (01) [right of = 10]{$0,1$};
            \node[state] (11) [right of = 01]{$1,1$};
            \node[state] (Q1) [right of = 11]{$Q,1$};
            
            \path[every node/.style={font=\sffamily\small}]
                (00) edge[bend left] node{$\lambda$} (10)
                (01) edge[bend left] node{$\lambda$} (11)
                (01) edge[bend left] node{$\mu_2$} (00)
                (10) edge[bend left] node{$\mu_1$} (01)
                (11) edge[bend left] node{$\mu_2$} (01)
                (11) edge[bend left] node{$\mu_1$} (Q1)
                (Q1) edge[bend left = 55] node{$\mu_2$} (01)
            ;
        \end{tikzpicture}

        \caption{The CTMC of a tandem of M/M/1/0 devices.}
    \end{figure}
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[->,>=stealth,shorten >=1pt,auto,node distance=2.5cm,thick,main node/.style={scale=0.8, circle,draw,font=\sffamily\normalsize}]
            ...
            \node[state] (11) [right of = 01]{$1,1$};
            \node[state] (Q1) [right of = 11]{$Q,1$};
            \node[circle, draw, , dashed, color = gray, minimum size = 50] (x) [left of = 11]{};
        ...
        \caption{Application of the barrier technique on state $(0,1)$.}
    \end{figure}

    We derive the stationary equations proceeding in the same way on all states:
    \[\soe{rlcl}{
        \lambda \pi_{0,0} &= \mu_2 \pi_{0,1} \\
        \mu_1 \pi_{1,0} &= \lambda \pi_{0,0} \\
        (\lambda + \mu_2) \pi_{0,1} &= \mu_1 \pi_{1,0} + \mu_2 (\pi_{1,0} + \pi_{Q,1}) \\
        (\mu_1 + \mu_2) \pi_{1,1} &= \lambda \pi_{0,1} \\
        \mu_2 \pi_{Q,1} &= \mu_1 \pi_{1,1} \\
        \sum\limits_{i \in \mathcal{I}} \pi_i &= 1
    }\]

    obtaining the following solution:
    \[\pi_{0,0} = \frac{\mu_2^2}{\lambda^2 + \lambda\mu + \mu^2}\]
    \[\pi_{1,0} = \frac{\lambda}{\mu_2} \cdot \frac{\mu_2^2}{\lambda^2 + \lambda\mu + \mu^2} \qquad\quad \pi_{0,1} = \frac{\lambda}{\mu_1} \cdot \frac{\mu_2^2}{\lambda^2 + \lambda\mu + \mu^2}\]
    \[\pi_{1,1} = \frac{\lambda^2}{\mu_2(\mu_1+\mu_2)} \cdot \frac{\mu_2^2}{\lambda^2 + \lambda\mu + \mu^2} \qquad\quad \pi_{Q,1} = \frac{\mu_1\lambda^2}{\mu_2^2(\mu_1+\mu_2)} \cdot \frac{\mu_2^2}{\lambda^2 + \lambda\mu + \mu^2}\]

    Let's see another example. Consider a batch system formed by a CPU with an infinite buffer linked to two equal disks that share the same infinite buffer, where $\mu_{\text{CPU}} = 4$ and let $\mu_{\text{Disk A}} = \mu_{\text{Disk B}} = 1.5$. The total number of jobs in the system is known to be $N = 3$. We observe that, since they are equal, the two disks can be considered as a M/M/2 device, while the CPU can be considered as an M/M/1 device.

    \begin{figure}[H]
        \centering
        \includegraphics[scale=0.75]{images/tandem_2.png}
        \caption{A batch system formed by an M/M/1 device and an M/M/2 device.}
    \end{figure}

    Since the number of jobs in the system is fixed to $N = 3$, we can model the state space as $\mathcal{I} = \{(n_1, n_2) \mid n_1, n_2 \in \N, n_1 + n_2 = 3\}$. Therefore, we can model the following CTMC for this problem.
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[->,>=stealth,shorten >=1pt,auto,node distance=2.5cm,thick,main node/.style={scale=0.8, circle,draw,font=\sffamily\normalsize}]
            ...
            \node[state] (30) []{$3,0$};
            \node[state] (21) [right of = 30]{$2,1$};
            \node[state] (12) [right of = 21]{$1,2$};
            \node[state] (03) [right of = 12]{$0,3$};
            
            \path[every node/.style={font=\sffamily\small}]
                (30) edge[bend left] node{$4$} (21)
                (21) edge[bend left] node{$4$} (12)
                (12) edge[bend left] node{$4$} (03)
                (03) edge[bend left] node{$2 \cdot 1.5$} (12)
                (12) edge[bend left] node{$2 \cdot 1.5$} (21)
                (21) edge[bend left] node{$1.5$} (30)
            ;
        \end{tikzpicture}

        \caption{The CTMC of the above batch system.}
    \end{figure}

    By applying the barrier technique, we get the following limiting distribution:
    \[\pi_{3,0} = 0.08 \qquad \pi_{2,1} = 0.22 \qquad \pi_{1,2} = 0.3 \qquad \pi_{0,3} = 0.4\]

    To compute the throughput of the whole batch system, we can proceed with the same reasoning used in the first chapter: every job must pass through the CPU (or through the ensemble of the two disks), thus $X = X_{\text{CPU}}$. Since the CPU is a M/M/1 device, from the analysis done in the previous section we know that:
    \[X_{\text{CPU}} = U_{\text{CPU}} \mu_{\text{CPU}} = (\pi_{3,0} + \pi_{2,1} + \pi_{1,2}) \mu_{\text{CPU}} = 2.4 \text{ jobs/sec}\]

    Lastly, we show how to work with interactive systems. Consider an interactive system with $N = 3$ users and a single CPU that loops jobs into itself with probability $0.5$. The think time is $Z \sim \mathrm{Exp}(\lambda = 1)$, while the service time of the CPU is $S \sim \mathrm{Exp}(\mu = 2)$.

    \begin{figure}[H]
        \centering
        \includegraphics[scale=0.75]{images/tandem_3.png}
        \caption{An interactive system with an looping M/M/1 device.}
    \end{figure}

    We observe that, even thought it loops into itself, the CPU can still be represented as an M/M/1 device whose population decreases with rate $\mu/2$. Moreover, we observe that the rate with which the population increases changes based on the number of jobs that lie inside the think center, which is equal to $N- N_{\text{CPU}}$. We can model this problem through the following CTMC. 
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[->,>=stealth,shorten >=1pt,auto,node distance=2.5cm,thick,main node/.style={scale=0.8, circle,draw,font=\sffamily\normalsize}]
            ...
            \path[every node/.style={font=\sffamily\small}]
                ...
                (3) edge[bend left] node{$\frac{\mu}{2}$} (2)
                (2) edge[bend left] node{$\frac{\mu}{2}$} (1)
                (1) edge[bend left] node{$\frac{\mu}{2}$} (0)
            ;
        \end{tikzpicture}

        \caption{The CTMC of the above interactive system.}
    \end{figure}

    By applying the barrier technique, we get the following limiting distribution:
    \[\pi_{0} = \frac{1}{16} \qquad \pi_{1} = \frac{3}{16} \qquad \pi_{2} = {\frac{6}{16}} \qquad \pi_{3} = \frac{6}{16}\]

    \section{Jackson networks}

    A Jackson network is a very general form of queueing network. In a Jackson network, there are $k$ devices, each with its own unbounded queue -- therefore they are $k$ M/M/1 devices. Jobs at a server are served in FCFS order. The ith server has service rate $\mathrm{Exp}(\mu_i)$. Each server may receive arrivals from inside and outside the network. The arrivals into the $i$-th server from outside the network constitute a Poisson process with rate $r_i$ . The routing of jobs is proba bilistic. Specifically, every job that completes at server $i$ will be transferred to server $j$ with probability $P_{i,j}$, or will exit the system with probability $P_{i,\text{out}} = 1 - \sum\limits_{j = 1}^k P_{i,j}$.

    \begin{figure}[H]
        \centering
        \includegraphics[scale=0.75]{images/jackson.png}
        \caption{A portion of a Jackson network.}
    \end{figure}

    In a similar fashion to what we have already seen in the previous chapter, we model the state space as $\mathcal{I} = \{(n_1, \ldots, n_k) \mid n_i \in \N\}$, where each $n_i$ is the population of device $i$. Each state $(n_1, \ldots, n_i, \ldots, n_j, \ldots, n_k)$ has three types of transitions: transitions to states where the global population decreases, transitions to states where it increases and transition to states where it stays the same. A transition of the first type is reached when a job leaves the system, in particular the value $n_j$ of the $j$-th entry will decrease to $n_j-1$. Similarly, a transition of the second type is reached when a job enters the system, increasing $n_j$ to $n_j+1$. Lastly, a transition of the third type is reached when a job passes from a device to another.
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[->,auto,node distance=2.5cm,thick,main node/.style={scale=0.8, circle,draw}]
            
            \node[ellipse, draw, minimum height = 40] (0) []{\LARGE $n_1, \ldots, n_i, \ldots, n_j, \ldots, n_k$};
            
            \node[ellipse, draw, minimum height = 40] (1) [below of = 0, yshift=-100]{\LARGE $n_1, \ldots, n_i-1, \ldots, n_j+1, \ldots, n_k$};
            \node[ellipse, draw, minimum height = 40] (2) [above of = 0, yshift=100]{\LARGE $n_1, \ldots, n_i+1, \ldots, n_j-1, \ldots, n_k$};
            
            \node[ellipse, draw, minimum height = 40] (3) [below left of = 0, xshift=-300, yshift=-25]{\LARGE $n_1, \ldots, n_i-1, \ldots, n_j, \ldots, n_k$};
            \node[ellipse, draw, minimum height = 40] (4) [above left of = 0, xshift=-300, yshift=25]{\LARGE $n_1, \ldots, n_i, \ldots, n_j-1, \ldots, n_k$};
            
            \node[ellipse, draw, minimum height = 40] (5) [below right of = 0, xshift=300, yshift=-25]{\LARGE $n_1, \ldots, n_i+1, \ldots, n_j, \ldots, n_k$};
            \node[ellipse, draw, minimum height = 40] (6) [above right of = 0, xshift=300, yshift=25]{\LARGE $n_1, \ldots, n_i, \ldots, n_j+1, \ldots, n_k$};
            
            \path[every node/.style={font=\sffamily\LARGE}]
                (0) edge[bend left] node{$\mu_i P_{i,j}$} (1)
                (0) edge[bend left] node{$\mu_j P_{j,i}$} (2)
                (0) edge[bend left] node{$\mu_j P_{j,\text{out}}$} (3)
                (0) edge[bend left] node{$\mu_i P_{i,\text{out}}$} (4)
                (0) edge[bend left] node{$r_j$} (5)
                (0) edge[bend left] node{$r_i$} (6)

                (1) edge[bend left] node{$\mu_j P_{j,i}$} (0)
                (2) edge[bend left] node{$\mu_i P_{i,j}$} (0)
                (3) edge[bend left] node{$r_j$} (0)
                (4) edge[bend left] node{$r_i$} (0)
                (5) edge[bend left] node{$\mu_j P_{j,\text{out}}$} (0)
                (6) edge[bend left] node{$r_j P_{i,\text{out}}$} (0)
            ;
        \end{tikzpicture}
        }

        \caption{A portion of the CTMC modeling a generic Jackson network.}
    \end{figure}

    It's easy to see that the full CTMC that models a Jackson network is way more complex than anything that we've seen until now. Clearly, we need to find a way to tackle this beast. Fix a generic state $(n_1, \ldots, n_k)$. By applying the barrier technique only on this state -- as done in the previous section -- we get that the outgoing rate $\mathcal{X}_{(n_1, \ldots, n_k)}$ must be equal to the ingoing rates $\mathcal{Y}_{(n_1, \ldots, n_k)}$. For the outgoing rates, we have that:
    \[\begin{split}
        \mathcal{X}_{(n_1, \ldots, n_k)} &= \sum\limits_{i = 1}^k \underbrace{\pi_{n_1, \ldots, n_k} r_i}_{\substack{\text{arrivals to device $i$} \\ \text{from outside}}} + \sum\limits_{i = 1}^k \underbrace{\pi_{n_1, \ldots, n_k} \mu_i P_{i,\text{out}}}_{\substack{\text{departures from device $i$} \\ \text{to outside}}} + \sum\limits_{i = 1}^k \sum\limits_{\substack{j = 1 \\ j \neq i}}^k \underbrace{\pi_{n_1, \ldots, n_k} \mu_i P_{i,j}}_{\substack{\text{internal transition from} \\ \text{device $i$ to device $j$}}} \\
        &= \sum_{i = 1}^k \pi_{n_1, \ldots, n_k} r_i + \sum_{i = 1}^k \pi_{n_1, \ldots, n_k} \mu_i (1-P_{i,i}) \\
        &= A + \sum_{i = 1}^k B_i
    \end{split}\]

    where:
    \[A = \sum_{i = 1}^k \pi_{n_1, \ldots, n_k} r_i \qquad B_i = \pi_{n_1, \ldots, n_k} \mu_i (1-P_{i,i})\]

    By definition, $A$ represents the rate of leaving state $(n_1 , \ldots , n_k)$ due to an outside arrival coming into the network, while $B_i$ represents the rate of leaving the state due to a departure from server $i$ (either to some other server $j \neq i$ or to the outside).

    For the incoming rates, instead, we have that:
    \[\begin{split}
        \mathcal{Y}_{(n_1, \ldots, n_k)} &= \sum\limits_{i = 1}^k \underbrace{\pi_{n_1, \ldots, n_i-1, \ldots, n_k} r_i}_{\substack{\text{arrivals to device $i$} \\ \text{from outside}}} + \sum\limits_{i = 1}^k \underbrace{\pi_{n_1, \ldots, n_i+1, \ldots, n_k} \mu_i P_{i,\text{out}}}_{\substack{\text{departures from device $i$} \\ \text{to outside}}} + \sum\limits_{i = 1}^k \sum\limits_{\substack{j = 1 \\ j \neq i}}^k \underbrace{\pi_{n_1, \ldots, n_i-1, \ldots,n_j+1, \ldots, n_k} \mu_j P_{j,i}}_{\substack{\text{internal transition from} \\ \text{device $j$ to device $i$}}} \\
        &= \sum\limits_{i = 1}^k \pi_{n_1, \ldots, n_i+1, \ldots, n_k} \mu_i P_{i,\text{out}} + \sum\limits_{i = 1}^k \rbk{\pi_{n_1, \ldots, n_i-1, \ldots, n_k} r_i + \sum\limits_{\substack{j = 1 \\ j \neq i}}^k \pi_{n_1, \ldots, n_i-1, \ldots,n_j+1, \ldots, n_k} \mu_j P_{j,i}} \\
        &= A' + \sum_{i = 1}^k B_i'
    \end{split}\]

    where:
    \[A' = \sum\limits_{i = 1}^k \pi_{n_1, \ldots, n_i+1, \ldots, n_k} \mu_i P_{i,\text{out}} \qquad B_i' = \pi_{n_1, \ldots, n_i-1, \ldots, n_k} r_i + \sum\limits_{\substack{j = 1 \\ j \neq i}}^k \pi_{n_1, \ldots, n_i-1, \ldots,n_j+1, \ldots, n_k} \mu_j P_{j,i}\]

    Clearly, the full system with all the equations $\mathcal{X}_{(n_1, \ldots, n_k)} = \mathcal{Y}_{(n_1, \ldots, n_k)}$ for each state $(n_1, \ldots, n_k) \in \mathcal{I}$ is way too complex to solve. We need an approach to simplify the huge number of balance equations. Our approach is based on \textbf{local balance}. The idea is to break down the left-hand side and right-hand side of the balance equation into $k + 1$ matching components, showing that $A = A'$ and $B_i = B_i'$ for each $i \in [k]$.

    \begin{figure}[H]
        \centering
        \includegraphics[scale=0.75]{images/jackson_2.png}
        \caption{Local balance decomposition approach}
    \end{figure}
    
    If we can find a solution that maintains the equality for each matching component (\textit{local balance}), then we know that it is a solution to the equation as a whole (\textit{global balance}). Naturally, satisfying local balance is a stronger condition than satisfying global balance: satisfying each local balance implies satisfying the whole global balance, but the converse isn't true.

    We start by showing that $A = A'$. By applying conditional probability, we have that:
    \[\pi_{n_1, \ldots, n_i, \ldots n_k} = \Pr[\text{reach $(n_1, \ldots, n_i, \ldots, n_k)$} \mid \text{from $(n_1, \ldots, n_i-1, \ldots, n_k)$}] \cdot \pi_{n_1, \ldots, n_i-1, \ldots n_k}\]

    for all $i \in [k]$. It seems reasonable to assume that:
    \[\Pr[\text{next state $(n_1, \ldots, n_i, \ldots, n_k)$} \mid \text{curr. state $(n_1, \ldots, n_i-1, \ldots, n_k)$}] = c_i\]
    for some constant $c_i$, i.e. that this probability doesn't change over time. In other words, we're assuming that:
    \[\pi_{n_1, \ldots, n_i, \ldots n_k} = c_i \pi_{n_1, \ldots, n_i-1, \ldots n_k}\]

    We can now use this assumption to show that:
    \[\begin{split}
        A = A' &\iff \sum_{i = 1}^k \pi_{n_1, \ldots, n_k} r_i = \sum\limits_{i = 1}^k \pi_{n_1, \ldots, n_i+1, \ldots, n_k} \mu_i P_{i,\text{out}}\\
        &\iff \sum_{i = 1}^k \pi_{n_1, \ldots, n_k} r_i = \sum\limits_{i = 1}^k c_i \pi_{n_1, \ldots, n_i, \ldots, n_k} \mu_i P_{i,\text{out}} \\
        &\iff \sum_{i = 1}^k r_i = \sum\limits_{i = 1}^k c_i \mu_i P_{i,\text{out}} \\
    \end{split}\]

    We'll now make another reasonable assumption: we can guess that $c_i = \rho_i = \lambda_i / \mu_i$, meaning that the probability of having an increase in device $i$ depends only on the arrival and departure rates of the device itself (as if we were applying the barrier technique). This final assumption allows us to infer that:
    \[\begin{split}
        A = A' &\iff \sum_{i = 1}^k r_i = \sum\limits_{i = 1}^k c_i \mu_i P_{i,\text{out}} \\
        &\iff \sum_{i = 1}^k r_i = \sum\limits_{i = 1}^k \lambda_i P_{i,\text{out}}\\
    \end{split}\]

    which simply says that the total rate of jobs entering the system from outside is the same as the total rate of jobs leaving the system. This is obviously true in steady state, concluding that $A = A'$ holds under these two assumptions. Let's now focus on showing that $B_i = B_i'$ for all $i \in [k]$. First, we observe that by repeatedly applying our last assumption we get that:
    \[\begin{split}
        \pi_{n_1, \ldots, n_j, \ldots, n_k} &= \rho_i \pi_{n_1, \ldots, n_j-1, \ldots, n_k} = \rho_j^{n_j} \pi_{n_1, \ldots, 0, \ldots, n_k} = \pi_{0, \ldots, 0} \prod_{h = 1}^k \rho_h^{n_h}\\
    \end{split}\]

    This allows us to infer that:
    \[\begin{split}
        B_i = B_i' &\iff \pi_{n_1, \ldots, n_k} \mu_i (1-P_{i,i}) = \pi_{n_1, \ldots, n_i-1, \ldots, n_k} r_i + \sum\limits_{\substack{j = 1 \\ j \neq i}}^k \pi_{n_1, \ldots, n_i-1, \ldots,n_j+1, \ldots, n_k} \mu_j P_{j,i} \\
        &\iff \mu_i (1-P_{i,i}) \pi_{0, \ldots, 0} \prod_{h = 1}^k \rho_h^{n_h} = \frac{r_i}{\rho_i} \pi_{0, \ldots, 0} \prod_{h = 1}^k \rho_h^{n_h} + \frac{\rho_j}{\rho_i} \sum\limits_{\substack{j = 1 \\ j \neq i}}^k \rbk{\mu_{j}P_{j,i} \pi_{0, \ldots, 0} \prod_{h = 1}^k \rho_h^{n_h}} \\
        &\iff \mu_i (1-P_{i,i}) = \frac{r_i}{\rho_i} + \frac{\rho_j}{\rho_i} \sum\limits_{\substack{j = 1 \\ j \neq i}}^k \mu_{j}P_{j,i} \\
        &\iff \lambda_i (1-P_{i,i}) = r_i + \sum\limits_{\substack{j = 1 \\ j \neq i}}^k \lambda_jP_{j,i} \\
        &\iff \lambda_i = r_i + \sum\limits_{j = 1}^k \lambda_jP_{j,i} \\
    \end{split}\]

    which simply says that the total rate of jobs entering device $i$ is equal to the rate of jobs entering device $i$ from outsite and the total rate of jobs entering device $i$ from other devices, which is clearly true. Therefore, $B_i = B_i'$ also holds.

    Let's take a step back. We said that: 
    \[\begin{split}
        \pi_{n_1, \ldots, n_k} = \pi_{0, \ldots, 0} \prod_{h = 1}^k \rho_h^{n_h}\\
    \end{split}\]

    Since the limiting distribution of the CTMC of the Jackson network must sum up to 1, we have that:
    \[\begin{split}
        1 &= \sum_{(n_1, \ldots, n_k) \in \mathcal{I}} \pi_{n_1, \ldots, n_k} \\
        &= \pi_{0,\ldots,0} \sum_{(n_1, \ldots, n_k) \in \mathcal{I}} \prod_{h = 1}^k \rho_h^{n_h}\\
        &= \pi_{0,\ldots,0} \sum_{(n_1, \ldots, n_k) \in \mathcal{I}} \prod_{h = 1}^k \rho_h^{n_h}\\
        &= \pi_{0,\ldots,0}  \prod_{h = 1}^k \sum_{n_h = 1}^{+\infty} \rho_h^{n_h}\\
        &= \pi_{0,\ldots,0}  \prod_{h = 1}^k \frac{1}{1-\rho_h}\\
    \end{split}\]

    which concludes that $\pi_{0, \ldots, 0} = (1-\rho_1) \cdot \ldots \cdot (1-\rho_k)$. Now, we observe that since each device is a M/M/1 queueing system, for each $i \in [k]$ it holds that $\pi_0[i] = 1-\rho_i$, where $\pi_0[i]$ denotes the probability that the $i$-th device is in state $0$. Putting everything together, we have that:
    \[\begin{split}
        \pi_{n_1,\ldots, n_k} &=  \pi_{0, \ldots, 0} \prod_{h = 1}^k \rho_h^{n_h} = \prod_{h = 1}^k \pi_0[h] \rho_h^{n_h} = \prod_{h = 1}^k \pi_{n_h}[h]
    \end{split}\]

    \begin{framedthm}{Product form theorem}
        Consider a Jackson network with $k$ devices. Then, for each state $(n_1, \ldots, n_k) \in \mathcal{I}$ it holds that:
        \[\pi_{n_1, \ldots, n_k}= \prod_{h = 1}^k \pi_{n_h}[h] = \prod_{h = 1}^k \rho_h^{n_h}(1-\rho_h)\]
    \end{framedthm}

    The latter theorem and the local balance equations allows us to compute the limiting distribution in a Jackson network with $k$ devices through the following system:
    \[\soe{rlcl}{
        \lambda_i &= r_i + \sum\limits_{j = 1}^k \lambda_jP_{j,i} & \qquad & \forall i \in [k] \\
        \rho_i &= \frac{\lambda_i}{\rho_i} & \qquad & \forall i \in [k] \\
        \pi_{n_1, \ldots, n_k} &= \prod\limits_{h = 1}^k \rho_h^{n_h}(1-\rho_h) & \qquad & \forall (n_1, \ldots, n_k) \in \mathcal{I}
    }\]

    For instance, consider a web server that receives requests for files according to a Poisson arrival process of rate $\lambda$. Each request requires alternating between the CPU and I/O some geometrically distributed number of times as the file is segmented into packets and sent to the network.

    \begin{figure}[H]
        \centering
        \includegraphics[scale=0.8]{images/jackson_3.png}
        \caption{The web server example.}
    \end{figure}

    We want to find the limiting probabilities of this Jackson network, thus we can use the previous system to find our request. We start by finding the values of $\lambda_1$ and $\lambda_2$ by solving the following subsystem:
    \[\soe{ll}{
        \lambda_1 &= \lambda + \lambda_2 \\
        \lambda_2 &= (1-p) \lambda_1 \\
    }\]

    which yields the solutions $\lambda_1 = \frac{\lambda}{p}$ and $\lambda_2 = \frac{\lambda (1-p)}{p}$. Therefore, we also get that:
    \[\rho_1 = \frac{\lambda_1}{\mu_1} = \frac{\lambda}{p \mu_1} \qquad \rho_2 = \frac{\lambda_2}{\mu_2} = \frac{\lambda(1-p)}{p \mu_2}\]

    and finally that:
    \[\pi_{n_1, n_2} = \rho_1^{n_1}\rho_2^{n_2}(1-\rho_1)(1-\rho_2) = \rbk{\frac{\lambda_1}{\mu_1}}^{n_1} \rbk{\frac{\lambda(1-p)}{p \mu_2}}^{n_2}\rbk{1-\frac{\lambda}{p \mu_1}}\rbk{1-\frac{\lambda(1-p)}{p \mu_1}}\]

    If we want to compute $\Exp[N]$, we can simply use the fact that $N = N_1 + N_2$ and that both devices are M/M/1 queueing systems to obtain that:
    \[\Exp[N] = \Exp[N_1] + \Exp[N_2] = \frac{\rho_1}{1-\rho_1} + \frac{\rho_2}{1-\rho_2}\]

    Other performance measures can be computed in similar ways.

    \section{Entry-specific response time in Jackson networks}

    Since jobs can enter a Jackson network from multiple entry points, we're often more interested in the \textbf{entry-specific response time} instead of a \textit{global response time}. This new performance metric, often denoted with $\Exp[T_i^{\text{tot}}]$ for device $i$, describes the mean system response time for a packet entering at device $i$. We remark that this metric is \underline{different} from the device response time $\Exp[R_i]$ (or $\Exp[T_i]$ for closed devices): the latter metric describes \textit{only} the sum of waiting time and service time inside device $i$, while the new metric describes the average time required by a job entering at device $i$ to leave the \underline{whole} system, not only device $i$.

    From the above explanation, it should be immediate to think of $\Exp[T_i^{\text{tot}}]$ as some kind of \curlyquotes{recursive metric}: the entry-specific response time of device $i$ will clearly depend on the entry-specific response time of every device in the network (since packets entering from device $i$ may not be guaranteed to go out of the system), which in turn will depend on the entry-specific response time of device $i$ (and the other devices themselves). Indeed, this can also formally proven.

    \begin{framedthm}{Recursive entry-specific response time}
        Given a device $i$ inside a Jackson network, it holds that:
        \[\Exp[T_i^{\text{tot}}] = \Exp[R_i] + \sum_{\substack{j = 0}}^{n} p_{i,j} \Exp[T_j^{\text{tot}}] \]
    \end{framedthm}

    \begin{proof}
        Let $K_i$ be the random variable denoting the next device where the current job of device $i$ is forwarded (we allow the special value $K_i = \text{out}$ to denote the fact that the packet left the system). Using the total formula of expected value, we get that:
        \[\Exp[T_i^{\text{tot}}] = p_{i, \text{out}}\Exp[T_i^{\text{tot}} \mid K = \text{ out}] + \sum_{j = 0}^n p_{i,j} \Exp[T_i^{\text{tot}} \mid K = j]\]

        Now, observe that $\Exp[T_i^{\text{tot}} \mid K = \text{ out}] = \Exp[R_i]$ since the job gets processed by device $i$ and then immediately leaves the network. Similarly, we have that $\Exp[T_i^{\text{tot}} \mid K = \text{ out}] = \Exp[R_i] + \Exp[T_j^{\text{tot}}]$ since the job gets processed by device $i$ and then gets forwarded to device $j$. Therefore, we conclude that:
        \[\Exp[T_i^{\text{tot}}] = p_{i, \text{out}}\Exp[R_i] + \sum_{j = 0}^n p_{i,j} (\Exp[R_i] + \Exp[T_j^{\text{tot}}]) = \Exp[R_i] + \sum_{j = 0}^n p_{i,j} \Exp[T_j^{\text{tot}}]\]
    \end{proof}

    Intuitively, the above theorem can be used to derive a system of linear equations by applying it on every device, which can be solved to get all the entry-specific response times. For instance, consider the following Jackson network where the exact value of $r_1$ is unknown.

    \begin{figure}[H]
        \centering
        \includegraphics[scale=0.65]{images/entry_specific_ex.png}
        \caption{A Jackson network}
    \end{figure}
    
    We start by computing the maximum possible value of $r_1$, namely $r_1^{\max}$. First, we setup the network equations given by the system using the procedure shown at the end of the previous section.
    \[\soe{ll}{
        \lambda_1 &= r_1 + \frac{1}{3} \lambda_2 \\
        \lambda_2 &= r_2 + \frac{1}{3} \lambda_1 + \frac{1}{3} \lambda_2
    }\]

    Solving the system, we get that:
    \[\soe{ll}{
        \lambda_1 &= \frac{6}{5} r_1 + \frac{3}{5} \\
        \lambda_2 &= \frac{3}{5} r_1 + \frac{9}{5}
    }\]

    Since $\lambda_1 < \mu_1 = 3$ and $\lambda_2 < \mu_2 = 5$ must hold in order for the system to be stable, we know that:
    \[\soe{ll}{
        3 &> \frac{6}{5} r_1 + \frac{3}{5} \\
        5 &> \frac{3}{5} r_1 + \frac{9}{5}
    } \implies \soe{ll}{
        r_1 &< 2 \\
        r_1 &< \frac{16}{3}
    } \implies r_1^{\max} = \min\rbk{2, \frac{16}{3}} = 2\]

    Now, suppose that we -- somehow -- get to know that $r_1 = 0.9 \cdot r_1^{\max} = \frac{9}{5}$. We can now compute the exact arrival rate of each device by simply substituting the exact value of $r_1$ in the previous system:
    \[\lambda_1 = \frac{6}{5} \cdot \frac{9}{5} + \frac{3}{5} = \frac{69}{25} \qquad\quad \lambda_2 = \frac{3}{5} \cdot \frac{9}{5} + \frac{9}{5} = \frac{72}{25} \]

    Using the M/M/1 formulas proven in \Cref{mmm1}, we conclude that:
    \[\Exp[R_1] = \frac{1}{\mu_1 - \lambda_1} = \frac{1}{3-\frac{69}{25}} = \frac{25}{6} \qquad\quad \Exp[R_2] = \frac{1}{\mu_2 - \lambda_2} = \frac{1}{5-\frac{72}{25}} = \frac{25}{53}\]
    
    We're now left with computing the values of $\Exp[T_1^{\text{tot}}]$ and $\Exp[T_2^{\text{tot}}]$. Using the previous theorem, we know that:
    \[\Exp[T_1^{\text{tot}}] = \Exp[R_1] + \frac{1}{3} \Exp[T_2^{\text{tot}}]\]
    \[\Exp[T_2^{\text{tot}}] = \Exp[R_2] + \frac{1}{3} \Exp[T_2^{\text{tot}}] + \frac{1}{3} \Exp[T_2^{\text{tot}}]\]

    To make things easier to read, let $X = \Exp[T_1^{\text{tot}}]$ and $Y = \Exp[T_2^{\text{tot}}]$. Then, we get the following system of linear equations:
    \[\soe{ll}{
        X = \frac{25}{6} + \frac{1}{3} Y \\
        Y = \frac{25}{53} + \frac{1}{3} X + \frac{1}{3} Y
    }\]

    which yields the solutions $X=\frac{280}{53}$ and $Y=\frac{355}{106}$.
