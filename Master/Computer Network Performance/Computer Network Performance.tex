...
\begin{document}
    ...
    \begin{framedprob}{}
        Consider an open system formed by a single server with an unbounded queue (see Figure 8.6). Suppose that, at every time step, with probabiliy $p_a$ a new job arrives to the server, while old jobs depart with probability $p_d$. Note that during a time step, we might have both an arrival and a transmission, or neither.
