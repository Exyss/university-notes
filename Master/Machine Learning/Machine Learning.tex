...
\begin{document}
        ...
        \begin{algorithmic}
            \Function{ID3}{$D,L$}
                ...
                \Else
                    ...
                    \For{$a \in \mathrm{Values}(A)$}
                        \State Let $D_a$ be the subset of $D$ whose tuples have $A$ set to $a$
                        \If{$D_a = \varnothing$}
    ...
    \[\mathrm{Gain}(S,\mathrm{Wind}) = \mathrm{Entropy}(S) - \frac{8}{14} \mathrm{Entropy}(S_\mathrm{Weak}) - \frac{6}{14} \mathrm{Entropy}(S_\mathrm{Strong}) \approx 0.048\]
    ...
    A common issue in decision tree learning is the \textbf{size} of the decision tree yield by the algorithm, that is the number of nodes in the output tree on average. The importance of size comes from the nature of the algorithm itself: since the hypothesis space is complete, if we allow the tree to have a huge number of nodes then it will eventually become a perfect approximation of the dataset.
    ...
    When we're working with a boolean random variable, i.e. when the value set $B$ is $\{0,1\}$, we often denote the event \curlyquotes{$X = 1$} with $X$, while the event \curlyquotes{$X = 0$} is denoted with $\lnot X$. This allows us to write intersections and unions of events as simple conjunctions and disjunctions of boolean random variables.
...
\end{frameddefn}
    ...
    The Optimal Bayes Classifier discussed in the previous section provides the best result, but it's not practical when the hypothesis space is large due to the necessity of enumerating all hypotheses. Instead, we usually work with a weaker version of this classifier, the \textbf{Naïve Bayes classifier}.
    ...
    \[\argmax_{C \in Y} \Pr[a_1, \ldots, a_n \mid C, D] \cdot \Pr[C \mid D] = \argmax_{C \in Y} \Pr[C \mid D] \prod_{a_i \in x}  \Pr[a_i \mid C,D]\]
    ...
    We also notice that, since we're interested in the solution maximizing the probability, these approximations don't have to be accurate. In fact, they must only try to preserve which solution will be chosen. For instance, the previous estimations preserve the maximum-likelihood solution.
    ...
    where $S_j = \frac{1}{N_j} \sum_{i \in C_j} (x_i - \widehat \mu_j)(x_i - \widehat \mu_j)^T$. Notice that as a byproduct of this process, we have found the parameters describing the Gaussian distributions of the samples of each class. This implies that we're now also able to generate new samples for each class!
    ...
    where $\alpha_h = \ln \Pr[x \mid C_h] \Pr[C_h]$. The solutions are:
    \[\begin{array}{ccc}
        \widehat \Pr[C_j \mid D] = \widehat p_j = \dfrac{N_j}{N} &\qquad\qquad& \Pr[x \mid C_j,D] = \mathcal{N}(x; \mu_j, \Sigma)\\\\
        \widehat \mu_j = \dfrac{1}{N_j} \sum\limits_{i = 1}^N t_{i,j} x_i &\qquad\qquad& \widehat \Sigma = \sum\limits_{j = 1}^k \dfrac{N_j}{N} S_j
    \end{array}\]

    where $S_j = \frac{1}{N_j} \sum_{i = 1}^N t_{i,j}(x_i - \widehat \mu_j)(x_i - \widehat \mu_j)^T$. After computing the values of the parameters, the prediction of a new sample $x \in X-X_D$ can be computed through the maximum likelihood:
    \[C^* \in \argmax_{C_j \in \mathcal{Y}} \frac{e^{\alpha_j}}{\sum_{h = 1}^k e^{\alpha_h}}\]
    where $\alpha_h = \pi_h \mathcal{N}(x; \mu_h, \Sigma)$. The use of matrix-like computation can be further extended by also modeling other elements as matrices. For instance, given a problem $f : \R^d \to Y$, a dataset $D = \{(x_i, t_i) \mid i \in [N]\}$ can also be represented as $\abk{X,t}$, where $X$ is an $N \times d$ matrix of input values and $t$ is a vector of $N$ output values.

    For all class-conditional distributions in the exponential family (e.g. Gaussian distributions) it is always possible to write $\alpha_j$ as $\alpha_j = w^Tx + w_0$ for some parameters $w, w_0$. In these parametric models, the maximum-likelihood solution for a $M_\Theta$ of dataset $D = \abk{X, t}$ with $\Pr[t \mid \Theta, X]$ is thus given by:
    ...
    \[\widetilde{w^*} \in \argmax_{\widetilde{w}} \ln \Pr[t \mid \widetilde{w}, X]\]

    A more specific type of probabilistic generative model is the \textbf{Gaussian Naïve Bayes}, where we use the same naive assumption of the Naïve Bayes Classifier. In particular, we have that:
    \[\Pr[x \mid C, D] = \prod_{j = 1}^n \Pr[a_j \mid C,D]\]
    where $x = (a_1, \ldots, a_n)$. With this assumption, we can model each attribute with its personal Gaussian, instead of single Gaussian that has to work with all of them:
    \[\Pr[C_i] = \pi_i \qquad \Pr[a_j \mid C_i, D] = \mathcal{N}(a_j; \mu_{j,i}, \sigma_{j,i})\]
    with $\pi_i, \mu_{j,i}, \sigma_{j,i}$ for all $i, j$. Now, the maximum likelihood solution becomes:
    \[\begin{array}{ccc}
        \widehat \Pr[C_i \mid D] = \pi_i = \dfrac{N_i}{N} \\
        \widehat \mu_{j,i} = \dfrac{1}{N_i} \sum\limits_{h = 1}^N t_{h,i} a_{j,h} \\ \widehat \sigma_{j,i} = \sum\limits_{h = 1}^N t_{h,i} (a_{j,h} - \mu_{j,i})^2 \
    \end{array}\]

    This assumptions allows us to get a smaller model that still performs well. For instance, consider a classification problem $f : \R^8 \to \{C_1, \ldots, C_4\}$. We want to compute the size of two models: 1) a probabilistic generative model with Gaussian distributions and different covariance matrices, 2) a Gaussian Naïve Bayes model.
    \begin{enumerate}
        \item We have a total of $3+32+144 = 179$ parameters, where $3$ parameters come from $\pi_1, \pi_2, \pi_3$ ($\pi_4$ can be computed by $\pi_4 = 1 - \pi_1 - \pi_2 - \pi_3$), $32$ parameters come from $\mu_1, \ldots, \mu_4$ ($4 \cdot 8$ parameters) and $144$ parameters come from $\Sigma_1, \ldots, \Sigma_4$ ($4 \cdot \frac{8(8+1)}{2}$)
        \item We have a total of $3+64= 68$ parameters, where $3$ parameters come from $\pi_1, \pi_2, \pi_3$ and $64$ parameters come from each $\mu_{j,k}$ and each $\sigma_{j,k}$ ($2 \cdot 4 \cdot 8$ parameters)
    \end{enumerate}
    ...
    without using Bayes' rule, where $\alpha_h = \widetilde{w}^T \widetilde{x}$ (this implies that we won't be able to generate new samples). In this case, the maximum-likelihood is still given by:
    \[\widetilde{w^*} \in \argmax_{\widetilde{w}} \ln \Pr[t \mid \widetilde{w}, X]\]
    ...
    This computation can also be made more compact and immediate -- thus the strength of this model:
    ...
    The functions describing such hyperplanes are called \textbf{linear discriminant functions}. Suppose that we had only two classes $C_1, C_2$. Then, the linear discriminant between these two classes is an hyperplane $y(x) = w^Tx + w_0 = \widetilde{w}^T \widetilde{x}$, where $\widetilde{w} = \smat{w \\ w_0}$ and $\widetilde{x} = \smat{x \\ 1}$, such that:
    \[\mathrm{Class}(x) = \soe{ll}{
        C_1 & \text{if } \widetilde{w}^T x \geq 0 \\
        C_2 & \text{if } \widetilde{w}^T x < 0
    }\]
    
    If we have more than two classes, more than one hyperplane is required in order to partition the whole space. There are two common approaches: \textbf{one-vs-one} and \textbf{one-vs-rest}. In the first approach, for each pair of classes $C_i, C_j$ we define an hyperplane separating $C_i$ from $C_j$. In the second approach, instead, for each class $C_i$ we define an hyperplane separating $C_i$ from all the other classes combined together. We'll focus on the second approach as it produces fewer hyperplanes ($k$ hyperplanes for $k$ classes instead of $\binom{k}{2}$ hyperplanes).

    If we compare the hyperplanes of two classes $C_i, C_j$, the hyperplane $(\widetilde{w_i} - \widetilde{w_j})^T x = 0$ acts as the boundary between the two classes. This is given by the fact that class $C_i$ \curlyquotes{wins} over $C_j$ when $\widetilde{w_i}^T x \geq \widetilde{w_j}^Tx$. By extending this comparison to all classes, we conclude that a sample $x$ is assigned to class $C_i$ when $\widetilde{w_i}^T x \geq \widetilde{w_j}^Tx$ for all $j \in [k]$. In other words, the class of $x$ is the class $C_i$  maximizing the value of $y_i(x)$.
    \[C^* \in \argmax_{C_j \in Y} y_j(x)\]
    ...
    Putting all the parameters of the hyperplanes together in a single matrix $\widetilde{W} = \smat{\widetilde{w_1} & \cdots & \widetilde{w_k}}$, learning a linear model is equivalent to estimating the matrix $\widetilde{W}$ for which $y(x) = \widetilde{W}^T \widetilde{x}$ defines the $K$-class discriminant.

    But what if the dataset is not linearly separable? In most cases, linear models can still be used for classification: the idea is to apply a non-linear transformation $\phi$ to the whole sample space that warps it into a linearly separable one. These linear transformations are called \textbf{basis functions}. After the samples spaces becomes linearly separable, linear models can be applied without problems. In other words, given a basis functions $\phi$, the linear model for the non-linearly separable dataset is given by the hyperplanes $y_i(x) = w_i^T \phi(x) + w_{i,0}$. The most common types of basis functions are:
    \begin{itemize}
        \item \textit{Linear}: $\phi(x) = \smat{x}^T$ with $x \in \R^d$
        \item \textit{Polynomial}: 
        \[\phi(\smat{x_1 & \cdots & x_n}^T) = \sbk{\begin{array}{cccccccccccc}
            1 & x_1 & x_1^2 & \cdots & x_1^M & x_2 & x_2^2 & \cdots & x_2^M & \cdots & \cdots & x_n^M
        \end{array}}^T\]
        with $x \in \R^d$ and for some $M \in \N$
        \item \textit{Radial Basis Function (RBF)}:
        \[\phi(x) = \smat{\exp(-\gamma \norm{x - \mu_1}^2) & \cdots & \exp(-\gamma \norm{x - \mu_M}^2)}^T\]
        for $x \in \R^d$, $\gamma \in \R$ and where $\mu_1, \ldots, \mu_M$ are $M$ cluster centers
    \end{itemize}

    \begin{figure}[H]
        \centering
        \includegraphics[scale=0.75]{images/basis_func.png}
        \caption{Example of a RBF with two cluster centers being applied to a non-linearly separable feature space.}
    \end{figure}

    Of course, the application of a basis function may (and this is usually the case) make us lose some information: there may be two original samples with different characteristics being mapped to a single sample that represents both. 

    \subsection{Least squares method}

    The first method that we'll discuss is the \textbf{least squares method}. Consider problem $f : \R^d \to Y$ a dataset $D = \{(x_i, t_i) \mid i \in [N]\}$, where each $t$ is a 1-of-$K$ encoding, meaning that $t_{i,j} = 1$ if $x_i \in C_j$, otherwise $t_{i,j} = 0$. We want to find the linear discriminant $y(x) = \widetilde{W}^T\widetilde{x}$. Let $\widetilde{X} = \smat{\widetilde{x_1} & \cdots & \widetilde{x_N}}^T$ and let $T = \smat{t_1 & \cdots & t_N}$. We notice that each row of the product $\widetilde{X} \widetilde{W}$ contains the predicted class for the associated input, while the matrix $T$ contains the real class for the input. When the two matrices are equal, the model is perfect. Hence, we want to minimize the difference between such matrices. To do so, we minimize the \textbf{sum-of-squares error function (SSE)}.

    \begin{frameddefn}{Sum-of-squares error (SSE) function}
    ...
    A \textbf{perceptron} is a simple linear binary classifier that assigns one of two classes to a real-valued input vector. In future sections, we'll see that the perceptron is actually the basis of modern neural networks. For now, we'll just focus on binary classification.
    
    Consider a linear binary classification problem $f : \R^d \to \{-1, +1\}$.  Let $w = \smat{w_0 & w_1 & \cdots & w_d}^T$ be a weight vector and let $x \in \R^{d+1}$ where $x_0 = 1$ -- from now on we'll drop the $\widetilde{w}, \widetilde{x}$ notation. The perceptron for $w$ is just the approximation function $o(x) : \R^d \to \{-1,+1\}$ defined as:
    \[o(x) = \mathrm{sign}(w^Tx) = \soe{ll}{
        +1 & \text{if } w_0 + w_1x_1 + \ldots w_dx_d \geq 0 \\
        -1 & \text{otherwise}
    }\]
    
    \begin{figure}[H]
        \centering
        \includegraphics[scale=0.9]{images/perceptron.eps} %
        \caption{Graphical representation of a perceptron.}
    \end{figure}

    We observe that this is nothing different from what we have already discussed for a generic linear binary classification: $w^Tx \geq 0$ we assign $x$ to class $+1$, otherwise we assign $x$ to class $-1$. We're just simplifying things by associating the two classes with the sign of the dot product. Therefore, we can just focus on the training rule of this model.

    Let $D = \{(x_i, t_i) \mid i \in [N]\}$ be our dataset. Consider the \underline{untresholded} variant of the perceptron model, i.e. the linear combination $w^Tx$ without the sign function applied to it. To derive a training rule for $o(x)$, we'll first derive a training rule for $w^Tx$ and then convert it into a training rule for the thresholded function. As done in the previous section, we want to minimize the SSE function (this time expressed in standard form instead of a matrix-trace form).
    \[E(w) = \frac{1}{2} \sum_{i = 1}^N (t_i - w^Tx)^2 \]

    Since we're working with real values, the minimization problem can be solved iteratively through \textbf{gradient descent}, i.e. the idea of iteratively moving towards the local minimum value. With some algebraic manipulation, we can find out that the partial derivative of each component of $E(w)$ is given by:
    \[\frac{\partial E}{\partial w_j} = \sum_{i = 1}^N (t_i - w^Tx_i)(-x_{j,i})\] 

    Consider some $\eta > 0$. Let $\Delta w_j = -\eta \frac{\partial E}{\partial w_j}$. The \underline{untresholded} perceptron training rule is simply given by iteratively applying the following instruction:
    \[w_j \gets w_j + \Delta w_j = w_j + \eta \sum_{i = 1}^N (t_i - w^Tx_i)x_{j,i}\]
    for a fixed number of iterations or until the value of $w_j$ doesn't change too much (say, until it doesn't change by a value below some $\varepsilon > 0$).

    Since this is nothing more than a standard gradient descent approach, this process is guaranteed to eventually reach a local minimum for the function $E(w)$.

    To get the \textbf{perceptron training rule}, we substitute the value of $w^Tx_i$ inside the untresholded training rule with the value of $\mathrm{sign}(w^Tx_i)$. 
    \[w_j \gets w_j + \Delta w_j = w_j + \eta \sum_{i = 1}^N (t_i - \mathrm{sign}(w^Tx_i))x_{j,i}\]

    We observe that what we obtain \underline{is not} a valid gradient descent procedure, but just thresholded version of the real gradient descent. In fact, it is impossible to get a gradient descent when using the thresholded perceptron due to the sign function being \textit{undifferentiable}. Nonetheless, this little modification suffices to achieve good enough results.
    
    Let's now focus of the value $\eta$. Notice that the choice of $\eta$ is \underline{critical}: if $\eta$ is small then each iteration will only slightly move the previous linear discriminant, while if $\eta$ is large each iteration may significantly move the discriminant. For this reason, the value $\nu$ is referred to as the \textbf{learning rate} of the model and it is considered an \textbf{hyperparameter}, i.e. a parameter chosen by the user that dictates how the learning process is achieved.

    It can be proven that the above procedure will always converge if the training data is linearly separable and if $\eta$ is sufficiently small. However, \underline{convergence doesn't imply termination}, hence the necessity of the terminating condition. Moreover, when $\eta$ is small, the final solution will either be very close to the samples of the first class or very close to the samples of the second class. In other words, the solution \textbf{cannot lie in the middle} of the two sample groups.  

    \begin{figure}[H]
        \centering
        \includegraphics[scale=0.8]{images/perceptron_2.png}
        \caption{Two iterations of the perceptron training rule with a small $\eta$ value. The vector $w$ is represented by the black arrow, while the vector $\Delta w$ is represented by the red arrow. Red samples that lie on the half-plane marked by the direction of the black arrow are correctly classified, while the green-circled samples are misclassified.}
    \end{figure}
    ...
    Consider a binary classification problem $f : \R^d \to \{+1, -1\}$ and a linearly separable dataset $D = \{(x_i,t_i) \mid i \in [N]\}$ where $t_i \in \{+1, -1\}$. Then, there is a function $y(x) = w^Tx + w_0$ such that forall $i \in [N]$ it holds that $y(x_i) > 0$ if $t_i = +1$ and $y(x_i) < 0$ if $t_i = -1$, implying that $\abs{y(x_i)} = t_i y(x_i)$ for all $i \in [N]$.
    ...
    We also notice that, since this doesn't affect the solution, we can also rescale all the points in such a way that the closest have unitary length, i.e. $\abs{y(x_i)} = 1$. Hence, we consider the canonical representation where $t_i(w^Tx + w_0) \geq 1$ for all $i \in [N]$. In this setup, the optimal solution is given by:
    \[w^*, w_0^* \in \argmax_{w, w_0} \frac{1}{\norm{w}} \min_{i \in [N]} t_i(w^Tx + w_0) = \argmax_{w, w_0} \frac{1}{\norm{w}} = \argmin_{w, w_0} \frac{1}{2} \norm{w}^2\]

    In other words, the problem is now equivalent to the following:
    \[w^*, w_0^* \in \argmin_{w, w_0} \frac{1}{2} \norm{w}^2\]
    subject to:
    \[t_i y(x_i) \geq 1 \qquad \forall i \in [N]\]
    
    This is a quadratic programming optimization problem, which can be solved through the Lagrangian method. For our interest, we won't dive into how this problem is solved. We care only about the fact that the solution is given by:
    \[w^* = \sum_{i = 1}^{N} a_i^* t_i x_i\]

    where each $a_i^*$ is a \textit{Lagrangian multiplier}, a byproduct of the following Lagrangian optimization problem:
    ...
    The optimization problem for determining $w^*,w_0^*$ ($d + 1$ dimensions) is thus transformed into an optimization problem for determining $a^*$ ($\abs{D}$ dimensions). This method is very efficient when $d \ll \abs{D}$ (the problem becomes sparse), and is also efficient when $d$ is large or infinite, as we reduce the size of the problem.
    ...
    Moreover, assuming that the observations $t_1, \ldots, t_N$ are i.i.d., the likelihood function is given by:
    ...
    \phi(x_2)_0 & \phi(x_2)_1 & \cdots & \phi(x_2)_M \\
    ...
    \[y({x}; \alpha) = \mathrm{sign}\rbk{w_0 + \sum_{i = 1}^N \alpha_i t_i x_i^T x}\]

    Applying the kernel trick we get that:
    \[y({x}; \alpha) = \mathrm{sign}\rbk{w_0 + \sum_{i = 1}^N \alpha_i t_i k(x_i,x)}\]
    ...
    \[E( w) = C \sum_{i = 1}^N E_{\varepsilon}(y_i, t_i) + \frac{1}{2}\norm{{w}}^2\]
    ...
    \[E_\varepsilon(y_i, t_i) = \soe{ll}{
        0 & \text{if } \abs{y_i - t_i} < \varepsilon \\
    ...
    \[\norm{x-x'}^2 = x^T x + {x'}^Tx' - 2x^Tx' \to k(x,x) + k(x',x') - 2 k(x,x')\]

    The K-NN algorithm can also be used to compute a \textbf{locally weighted regression}, a regression model formulated on the $K$-nearest neighbors of a query sample. The union of more locally weighted regressions can be used to estimate a rough regression model for the feature space.
    ...
    Given a dataset $D = \{(x_i,t_i) \mid i \in [N]\}$, the \textbf{voting} method first trains a set of models $y_1, \ldots, y_m$ on the same dataset to then make predictions using:
    \begin{itemize}
    ...
    A way to evaluate the complexity of an ensamble model is the \textbf{Bias-Variance Decomposition}. Let $y(x)$ be some observations over the learning problem, i.e. $y(x) = f(x) + \varepsilon$ for some small $\varepsilon \in \R$. Let $\widehat{f}(x; D)$ be the prediction of the model when trained on dataset $D$. Let also $S = \{(x_i, y_i)_{i}\}$ be the training set used. Recall that the MSE of the model is defined by:
    \[\mathrm{MSE} = \Exp_{(x_i, y_i) \in S}[(y_i - \widehat{f}(x_i; D))^2]\]
    
    This formula can be decomposed into three main components: \textit{bias}, \textit{variance} and \textit{noise}:
    \[\mathrm{MSE} = \mathrm{Bias}^2 + \mathrm{Variance} + \mathrm{Noise}\]

    where:
    \begin{itemize}
        \item $\mathrm{Bias} = \Exp_{(x_i, y_i) \in S}[\widehat{f}(x_i; D)] - f(x)$
        \item $\mathrm{Variance} = \Exp_{(x_i, y_i) \in S}[\widehat{f}(x_i; D) - \Exp_{(x_i, y_i) \in S}[(\widehat{f}(x_i; D)])^2] =$
        \item $\mathrm{Noise} = \Exp_{(x_i, y_i) \in S}[(y-f(x))^2]$
    \end{itemize}

    If the model has \textbf{high bias}, it must be that the model is making a lot of errors even on training data, i.e. it is underfitting the problem. If it has \textbf{high variance}, instead, the model may be highly able to perfectly adapt to the dataset, i.e. it will likely overfit the problem. If it has \textbf{high noise}, instead, the problem is too hard to approximate for the model.
    
    In other words, if a model has low complexity then it will have high bias due to it being too simplistic, while the variance will be huge if a model has high complexity. In general, it is impossible to minimize both bias and variance, requiring a \textit{tradeoff}. In ensemble methods, \textit{bagging} reduces variance, while \textit{boosting} reduce bias.
    ...
    \begin{framedalgo}{SDG with momentum}
        ...
        \begin{algorithmic}
            \Function{SDG\_Momentum}{$\eta, \mu, \theta^{(1)}$}
                \State{$k = 1$}
                \State{$v^{(1)} = 0$}
    ...
    When the \textbf{multiple kernels} are used, convolution produces multiple feature maps. For instance, if we have an input of size $32 \times 32 \time 1$ and we apply 2D convolution on 1 kernel with dimensions $5 \times 5 \times 1$ kernel, we get a feature map of size $28 \times 28$. Instead, if we apply 6 kernels with dimensions $5 \times 5 \times 1$, each kernel is individually applied on a copy of the input, producing 6 feature maps of size $28 \times 28$. These 6 feature maps are then represented by a $28 \times 28 \times 6$ tensor. In general, if we have an input of size
    ...
    where $w_{\mathrm{out}}$ and $h_{\mathrm{out}}$ are computed according to \textit{stride} and \textit{padding} (we'll see how in the next sections). For 3D convolutions, the constraint becomes $d_\mathrm{in} > d_\mathrm{k}$ as the output is a 4D tensor of size $w_{\mathrm{out}} \times h_{\mathrm{out}} \times z_{\mathrm{out}} \times d_{\mathrm{out}}$,  where $z_{\mathrm{out}}$ is also computed according to \textit{stride} and \textit{padding}.

    \subsection{Convolution in neural networks}

    As the name suggests, CNNs are just FNNs that use convolution in one or more layers. The learning phase finds which kernels are optimal for each \textbf{convolutional layer}. Each of these layers is formed by three sub-layers, called \textit{stages}:
    \begin{enumerate}
    ...
    The recent success of CNNs is yielded by continuous theoretical advancements in neural networks. However, they also have some downsides: they require massive parallel computing capacities to speed up computations (GPUs/TPUs), very large training sets (ImageNet/MS COCO) and developing frameworks (Keras, Theano, Tensorflow, PyTorch and others...). %this has been changed since in the compiled latex the three dots were alone in the next line
    ...
    A simple unsupervised learning model is the \textbf{Gaussian Mixture Model (GMM)}. Here, we assume that the dataset is generated by a mixed probability distribution, that is formed by the weighted average of $K$ different gaussian distributions.
    ...
    \[\Pr[x \mid z] = \prod_{k = 1}^K \mathcal{N}(x; \mu_k, \Sigma_k)^{z_k}\]
    ...
    \[S = \frac{1}{N} \sum_{i = 1}^N (x_i - \overline{x}) (x_i - \overline{x})^T = \frac{1}{N}X^TX\]
    ...
    A \textbf{dynamic system} is a system that evolves over time. At any given time $k$, a dynamic system has a \textit{state} $x_k$ representing the current configuration of the system.
    ...
    \[V^{\pi}(x) = \Exp \sbk{\sum_{i = 1}^{+\infty} \gamma_{i-1} \overline{r_i}} =  \sum_{i = 1}^{+\infty} \gamma_{i-1} \Exp \sbk{\overline{r_i}}\]
    ...
    \[\begin{split}
        ...
        &= (1-\alpha_{(t-1)}) \widehat{Q}_{(t-1)}(x, a) + \alpha_{(t-1)}(\overline{r} + \gamma \max_{a' \in A} \widehat{Q}_{(t-1)}(x', a'))
    \end{split}\]

    where $\alpha_{(t-1)} = \frac{1}{1+v_{(t-1)}(x,a)}$, with $v_{(t-1)}(x,a)$ being the total number of times the state-action pair $(x,a)$ has been visited up to the $t$-th iteration. 
    
    The new training rule is made up of two terms:
    \begin{itemize}
        \item $(1-\alpha_{(t-1)}) \widehat{Q}_{(t-1)}(x, a)$: this is the previous estimation of $Q^*(x,a)$ and is weighted by $1-\alpha_{(t-1)}$. If the pair $(x,a)$ has been visited a low number of times during the iterations, $1-\alpha_{(t-1)}$ will be near to zero and the new estimation will be more important.
        \item $\alpha_{(t-1)}(\overline{r} + \gamma \max_{a' \in A} \widehat{Q}_{(t-1)}(x', a'))$: this is the new estimation (except for $\alpha_{(t-1)}$, is identical to the non deterministic $Q$-learning rule) and have less value if the pair $(x,a)$ has been visited more times during the iterations. 
    \end{itemize}

    Through this rule, the non-deterministic Q-Learning algorithm is guaranteed to converge when every pair state-action is visited infinitely often.
    ...
    For non-deterministic learning there are also more advanced Q-Learning algorithms: the TD algorithm and the SARSA algorithm. \textbf{Temporal difference (TD)} learning is a variant of Q-learning where the discrepancy between successive Q estimates reduces over time. Given a value $n$, the $n$ step time difference of the function Q is given by:
    ...
    We observe that, in all variants, an explicit representation of $\widehat{Q}$ through a table may not be feasible for large models. In this case, we can define an approximating function $Q_\theta(x,a) = \theta_0 + \theta_1 F_1(x,a) + \ldots + \theta_n F_n(x,a)$ and use a linear regression to learn it, or directly training a neural network to approximate $\widehat{Q}$.
    ...
    The perturbations are generated from $\pi$ by setting $R_i = \{\theta_1 + \delta_1, \ldots, \theta_N + \delta_N\}$, with $\delta_j$ being randomly chosed from $\{-\varepsilon_j, 0, +\varepsilon_j\}$ for some small fixed value $\varepsilon_j$ relative to $\theta_j$.
