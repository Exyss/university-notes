...
\begin{document}
    ...
    \begin{framedalgo}{Regev's PKE scheme}
        ...
        \begin{itemize}
            \item $\mathrm{KGen}(1^\lambda) = (pk, sk)$ where $pk = \smat{\overline{A} & \vec b^T}^T$ and $sk = \vec s$, with each column of $A$ corresponding to a sample $(\overline{a_i}, b_i)$ with $b_i = \abk{\vec s, \overline{a_i}} + e_i \pmod q$
            \item $\mathrm{Enc}(pk, m) = \vec c$ where $m \in \{0,1\}$ and $\vec c = (pk) \vec r + \smat{\vec 0 & m \floor{q/2}}^T$, for some $\vec r \in_R \{0,1\}^m$
            \item $\mathrm{Dec}(pk, \vec c) = 1$ when $\smat{-\vec s, 1}^T \vec c \approx m \floor{q/2}$ and 0 otherwise
        \end{itemize}
    \end{framedalgo}
    ...
    \[\smat{- \vec s, 1}^T \vec c = \smat{- \vec s, 1}^T A \vec r + m \floor{\frac{q}{2}} = \vec e^T \vec r +  m \floor{\frac{q}{2}}\]
